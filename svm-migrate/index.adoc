...
| FAS | ONTAP 9.12.1 y posteriores 
|===
...
== Requisitos de rendimiento de la infraestructura de red para el tiempo de ida y vuelta (RTT) de TCP
...
| ONTAP 9.12.1 y posteriores | 10 ms 
|===
...
Antes de iniciar una migración de SMV, debe cumplir los siguientes requisitos previos:
...
* Todos los nodos del clúster de origen deben ejecutar ONTAP 9.10.1 o una versión posterior. Para obtener información sobre la compatibilidad de controladoras de cabina ONTAP específicas, consulte
...
* La red que conecta los clústeres de origen y destino debe tener un tiempo máximo de ida y vuelta (RTT) de no más de 10ms
...
| SnapMirror asíncrono de origen | ONTAP 9.12.1  a| 
...
| SVM de MetroCluster | No admitido |  
...
En la siguiente tabla, se indica la compatibilidad de la operación de volumen según el estado de migración:
...
| Cree | Permitido | Permitido | No admitido 


| Eliminar | No permitido | Permitido | No admitido 


| Análisis del sistema de archivos 2+| Consulte Análisis del sistema de archivos | No admitido 
...
| Qtreecreate/MODIFY | No permitido | No permitido | No admitido 
...
|===
En la siguiente tabla se indica la compatibilidad de la operación de archivos según el estado de migración:
