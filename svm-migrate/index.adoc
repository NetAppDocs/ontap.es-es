...
| SVM de MetroCluster | ONTAP 9.16.1 | Es posible migrar una SVM desde una pareja de HA que no sea MetroCluster a una configuración de MetroCluster o desde una configuración MetroCluster a una pareja de HA que no sea MetroCluster. No es posible migrar un SVM de una configuración de MetroCluster a otra de MetroCluster. [NOTA] La migración de SVM no admite la migración de SVM de MetroCluster en versiones anteriores a ONTAP 9.16,1. Es posible que pueda utilizar la replicación asíncrona de SnapMirror en link:https://www.netapp.com/media/83785-tr-4966.pdf["Migre una SVM en una configuración MetroCluster"^]. Debe tener en cuenta que el uso de SnapMirror Asynchronous para migrar una SVM en una configuración de MetroCluster es el método de migración _disruptive_. 


| Cifrado de agregados de NetApp (NAE) | No admitido | La migración no está soportada para ningún punto final que utilice NAE. 
