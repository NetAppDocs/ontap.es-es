...
NOTE: Cada puerto RDMA puede tener un máximo de 127 LIF. Cuando añades más LIF, o cuando un LIF pasa a un puerto que supera ese límite, los LIF recién añadidos o el LIF redirigido no pueden gestionar tráfico RDMA.
...
. Selecciona la VM de almacenamiento y el nodo de origen. Asigna un *Nombre*, una *dirección IP* y una *máscara de subred*.
...
** El parámetro  `-rdma-protocols` admite una lista que, por defecto, está vacía. Cuando  `roce` se añade como valor, la LIF solo se puede configurar en puertos compatibles con la descarga de RoCE, lo que afecta tanto a la migración de LIF como a la conmutación por error.
