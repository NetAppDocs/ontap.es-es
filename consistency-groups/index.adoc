...
Los grupos de coherencia ofrecen una protección coherente con las aplicaciones, lo que garantiza la coherencia de los datos en varios volúmenes o LIF. Al crear una copia Snapshot de un grupo de consistencia, se establece un «límite» en el grupo de consistencia. El cercado inicia una cola para I/O hasta que se completa la operación de Snapshot, lo que garantiza la coherencia de un momento específico de los datos en todas las entidades del grupo de coherencia. La operación de cercado puede provocar un pico transitorio en la latencia cuando(`-snapshot-consistency-type` se establece en `crash` (`-snapshot-consistency-type` es siempre `crash` Para operaciones de Snapshot programadas y basadas en políticas con grupos de coherencia).
