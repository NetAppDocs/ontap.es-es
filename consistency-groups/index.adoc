...
image:consistency-group-single-diagram.gif["Un grupo de coherencia único con sus volúmenes constituyentes y su política de Snapshot local"]
...
Los grupos de coherencia ofrecen una protección coherente con las aplicaciones, lo que garantiza la coherencia de los datos en varios volúmenes o LIF. Al crear una copia Snapshot de un grupo de consistencia, se establece un «límite» en el grupo de consistencia. El cercado inicia una cola para I/O hasta que se completa la operación de Snapshot, lo que garantiza la coherencia de un momento específico de los datos en todas las entidades del grupo de coherencia. Este cercado puede provocar un pico transitorio en la latencia durante las operaciones de creación de Snapshot, como una política de Snapshot programada o la creación de una snapshot con System Manager. Para obtener más información sobre el contexto de la API DE REST y la interfaz de línea de comandos, consulte la documentación de la API de REST DE ONTAP y la página del manual de la CLI.
...
NOTE: Las copias Snapshot creadas con los comandos ONTAPI `cg-start` y `cg-commit` no se reconocen como snapshots de grupo de consistencia y, por lo tanto, no se pueden gestionar mediante la interfaz de grupo de consistencia de System Manager ni los extremos del grupo de consistencia en la API REST DE ONTAP. A partir de ONTAP 9.14.1, estas copias Snapshot se pueden reflejar en el volumen de destino si utiliza una política asíncrona de SnapMirror. Para obtener más información, consulte xref:protect-task.html#configure-snapmirror-asynchronous[Configurar SnapMirror asíncrono].
