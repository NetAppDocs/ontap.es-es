...
. Marque la casilla junto a *Programar instantáneas (locales)*.
...
* Al iniciar la creación de una instantánea, puedes establecer un valor de tiempo de espera opcional de entre 5 y 120 segundos. Si no se especifica ningún valor de tiempo de espera, la operación caduca tras el valor predeterminado de 7 segundos. En la API de REST de ONTAP, establece el valor de tiempo de espera con el parámetro `action_timeout`. En la CLI de ONTAP, utiliza el indicador `-timeout`.


.Pasos
Puedes completar una instantánea en dos fases con la API de REST de ONTAP o, a partir de ONTAP 9.14.1, con la CLI de ONTAP. Esta operación no es compatible con System Manager.


NOTE: Si inicias la creación de una instantánea con la API de REST de ONTAP, debes confirmarla también con la API de REST de ONTAP. Si inicias la creación de una instantánea con la CLI de ONTAP, debes confirmarla también con la CLI de ONTAP. No se admite la combinación de métodos.
