...
---
= Agrega un volumen con ONTAP System Manager
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
+
** *Exportar a través de NFS*: Crea el volumen con la  `default` política de exportación que otorga a los usuarios acceso completo a los datos.
...
NOTE: También puedes guardar las especificaciones de este volumen en un Ansible Playbook. Para más detalles, ve a link:task_use_ansible_playbooks_add_edit_volumes_luns.html["Utilice libros de aplicaciones Ansible para añadir o editar volúmenes o LUN"].
