...
== Restringe el conjunto de dominios de Relying Party (RP) permitidos

A partir de ONTAP 9.19.1, los administradores deben usar el parámetro -rp-domains en el comando de configuración global WebAuthn. Esto permite a los administradores restringir explícitamente el conjunto de dominios de Relying Party (RP) permitidos que se aceptan durante la autenticación multifactor WebAuthn (FIDO2) en System Manager.

Los usuarios solo podrán acceder a la autenticación multifactorial (MFA) desde los dominios de RP autorizados.


NOTE: Si vas a actualizar a ONTAP 9.19.1 desde ONTAP 9.16.1 hasta 9.18.1, consulta link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-Issues/CONTAP-597688["CONTAP-597688"] para obtener más información.

[role="tabbed-block"]
====
.System Manager
--
. Selecciona *Cluster > Settings > Autenticación multifactor (MFA)*.
. Agregar, eliminar o modificar los dominios RP permitidos.


--
.CLI
--
. Modifica los dominios rp permitidos. Establece -rp-domains con el nombre de dominio completo (FQDN) del clúster:
+
[source, console]
----
...
----
+
Obtén más información sobre `security webauthn modify` en el link:https://docs.netapp.com/us-en/ontap-cli/security-webauthn-modify.html#parameters["Referencia de comandos del ONTAP"^].
