...
+
link:https://docs.netapp.com/us-en/ontap-afx/administer/revert-ontap.html["Sistemas de almacenamiento AFX"^]No admite reversión.
...
IMPORTANT: Si ha revertido algunos, pero no todos, los nodos, no intente actualizar el clúster a la versión de origen.
...
* En los sistemas IP de MetroCluster que utilizan conmutadores compatibles con MetroCluster pero no validados MetroCluster, la reversión de ONTAP 9.7 a 9.6 es disruptiva ya que no hay soporte para sistemas que utilizan ONTAP 9.6 y versiones anteriores.
...
+
Si intenta volver a una versión de ONTAP que no admite el cifrado del volumen raíz SVM, el sistema responderá con una advertencia y bloqueará la reversión.

* Si tienes habilitado NFS sobre TLS en ONTAP 9.19.1 o una versión posterior, debes desactivar todas las instancias de configuración de NFS sobre TLS antes de poder revertir a una versión anterior de ONTAP. NFS sobre TLS solo es compatible a partir de ONTAP 9.19.1.
