...
+
El `-version` La opción se refiere a la versión de ONTAP a la que está volviendo. Solo se requieren los dos primeros valores de la versión de ONTAP . Por ejemplo, si está volviendo de 9.14.1 a 9.13.1, el valor correcto de `-version` La opción es 9.13.
...
. Después de que ambos nodos hayan sido revertidos, vuelve a activar la alta disponibilidad (HA) para el clúster:
+
[source, cli]
----
...
NOTE: Al volver a habilitar la alta disponibilidad (HA) del clúster, es posible que el parámetro  `storage failover auto-giveback` vuelva a establecerse en true.
...
----
. Verifica la configuración de  `storage failover auto-giveback` en ambos nodos:
+
[source, cli]
----
...
----
+
Si necesitas una configuración diferente para tu entorno, utiliza el comando  `storage failover modify -node <nodename> -auto-giveback <true|false>` para modificarla.
...
* link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/Why_auto-giveback_enabled_after_re-enabling_storage_failover_setting["NetApp base de conocimientos: ¿Por qué se activa la devolución automática tras volver a habilitar la configuración de conmutación por error del almacenamiento?"^]
