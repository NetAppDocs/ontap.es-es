...
Trabajas con la configuración de `ext_performance_monitoring.enabled` usando el endpoint `/api/storage/qtrees`. Usa GET para obtener el valor actual, POST para establecerlo al crear un nuevo qtree y PATCH para cambiarlo en un qtree existente.
