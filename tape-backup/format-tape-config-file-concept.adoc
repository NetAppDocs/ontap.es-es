...
hasta 8 bytes
 a| 
El ID del proveedor según lo informa la `SCSI Inquiry` comando.
...
hasta 16 bytes
 a| 
El ID de producto indicado por la `SCSI Inquiry` comando.
...
hasta 16 bytes
 a| 
Si este parámetro está presente, se especifica mediante la cadena mostrada por el comando, `storage tape show -device-names`; De lo contrario, se mostrará INQ_VENDOR_ID.
...
hasta 16 bytes
 a| 
Si este parámetro está presente, se especifica mediante la cadena mostrada por el comando, `storage tape show -device-names`; De lo contrario, aparecerá INQ_PRODUCT_ID.
...
hasta 24 bytes
 a| 
La cadena que se va a imprimir para el comando nodeshell, `sysconfig -t`, que describe las características de la configuración de densidad determinada.
...
`autoload=(Boolean yes/no)`
 a| 
Este campo está establecido en `yes` si la unidad de cinta tiene una función de carga automática, es decir, después de insertar el cartucho de cinta, la unidad de cinta estará lista sin necesidad de ejecutar un `SCSI load` (unidad de arranque/parada). El valor predeterminado de este campo es `no`.
...
https://["Herramientas de NetApp: Archivos de configuración de dispositivos de cinta"]
