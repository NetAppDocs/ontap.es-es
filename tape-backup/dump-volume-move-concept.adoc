...
[role="lead"]
Las operaciones de backup en cinta y restauración, así como el traslado de volúmenes, pueden ejecutarse en paralelo hasta que el sistema de almacenamiento inicie la fase final de transición. Durante la fase final de transición, no se permiten nuevas operaciones de backup en cinta y restauración en el volumen que se está trasladando. Sin embargo, las operaciones en curso continúan ejecutándose hasta que terminan.
