...
---
= Habilitar el acceso del protocolo S3 a los datos NAS en un SVM de ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
.. Haz clic en *Almacenamiento > Máquinas virtuales de almacenamiento*, selecciona una máquina virtual de almacenamiento preparada para NAS, haz clic en *Configuración* y luego, en *Protocolos > S3*, haz clic en image:icon_gear.gif["El icono Actions"] en S3.
...
. Active S3 en una máquina virtual de almacenamiento con protocolos NAS configurados.
+
.. Haz clic en *Cluster > Storage VMs*, selecciona una storage VM preparada para NAS, haz clic en *Settings* y luego, en *Protocols > S3*, haz clic en image:icon_gear.gif["El icono Actions"].
.. Seleccione el tipo de certificado. Tanto si selecciona un certificado generado por el sistema como uno propio, será necesario para el acceso de los clientes.
.. Introduzca las interfaces de red.
...
+
** La clave secreta no se volverá a mostrar.
** Si necesitas volver a consultar la información del certificado: haz clic en *Cluster > Storage VMs*, selecciona la storage VM y haz clic en *Settings*.
