...
Puede definir una configuración de asignación de roles con `security login external-role-mapping create` el comando. Debe estar en el nivel de privilegio *admin* de ONTAP para emitir este comando, así como las opciones relacionadas. Obtenga más información sobre `security login external-role-mapping create` en el link:https://docs.netapp.com/us-en/ontap-cli/security-login-external-role-mapping-create.html["Referencia de comandos de la ONTAP"^].
