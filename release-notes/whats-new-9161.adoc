...
xref:../san-admin/enable-space-allocation.html[De forma predeterminada, la desasignación del espacio NVMe está habilitada]
 a| 
La desasignación de espacio (también llamada "perforación" y "desasignación") está habilitada para los espacios de nombres NVMe de manera predeterminada. La desasignación de espacio permite que un host desasigne bloques no utilizados de los espacios de nombres para recuperar espacio. Esto mejora enormemente la eficiencia general del almacenamiento, especialmente con sistemas de archivos que tienen una alta rotación de datos.
...
xref:../anti-ransomware/index.html[Protección autónoma frente a ransomware con mejoras de la IA (ARP/AI)]
 a| 
La protección autónoma contra ransomware se ha mejorado con nuevas capacidades de inteligencia artificial, lo que le permite detectar y responder a ataques de ransomware con una precisión y recuperación del 99 %. Como la IA está entrenada con un conjunto de datos completo, ya no hay un periodo de aprendizaje para ARP/AI ejecutándose en volúmenes FlexVol y ARP/AI comienza en modo activo de inmediato. ARP/AI también cuenta con una capacidad de actualización automática para asegurar protección y resiliencia constantes frente a las amenazas más recientes.
...
| Actualizar | Descripción 
 ...
 a| 
xref:../authentication-access-control/webauthn-mfa-overview.html[Soporte para la autenticación multifactor WebAuthn resistente al phishing en System Manager]
 a| 
