...
| En la API de REST de ONTAP, los grupos de consistencia admiten un procedimiento de instantáneas en dos fases, lo que permite realizar una comprobación previa antes de confirmar la instantánea. 
|===
...
Los administradores del Administrador del Sistema pueden especificar la configuración de un nivel local si no desean aceptar la recomendación del Administrador del Sistema. También pueden editar la configuración RAID de un nivel local existente.
