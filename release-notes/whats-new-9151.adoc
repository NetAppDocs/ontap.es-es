...
xref:../smb-admin/windows-backup-symlinks.html[Aplicaciones de copia de seguridad de Windows y enlaces simbólicos de estilo Unix en servidores]
 a| 
Cuando una aplicación de backup de Windows encuentra un enlace simbólico (symlink) de estilo Unix, se sigue el enlace y ONTAP devuelve los datos reales y los incluye en la copia de seguridad. A partir de ONTAP 9.15.1, también tienes la opción de hacer una copia de seguridad del propio enlace simbólico en lugar de los datos a los que apunta. Esto puede ofrecer varios beneficios, incluyendo un mejor rendimiento de tus aplicaciones de backup. Puedes habilitar la función usando la CLI de ONTAP o la API de REST de ONTAP.
...
Los grupos de consistencia que utilizan la protección asíncrona de SnapMirror ahora admiten hasta 80 volúmenes en el grupo de coherencia.
 ...
 a| 
xref:../consistency-groups/configure-task.html[Compatibilidad con el nivel de privilegio de administración para las operaciones de la API de REST de ONTAP y la CLI de ONTAP con grupos de consistencia]
 a| 
Las operaciones de la CLI de ONTAP y la API de REST de ONTAP para los grupos de consistencia ahora son compatibles a nivel de privilegio de administración.
...
xref:../concepts/ontap-and-vmware.html[Reservas persistentes para volúmenes virtuales de VMware con clustering de conmutación al nodo de respaldo de Windows Server]
 a| 
Actualmente, ONTAP es compatible con los volúmenes virtuales de VMware (vVols), así como con las reservas persistentes con LUN tradicionales. A partir de ONTAP 9.15.1, también puedes crear una reserva persistente con un vVol. La compatibilidad con esta función está implementada en ONTAP tools for VMware vSphere 9. Solo es compatible en un Windows Server Failover Cluster (WSFC), que es un grupo de máquinas virtuales Windows en clúster.
