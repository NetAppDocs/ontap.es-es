...
| Protección autónoma contra ransomware (ARP/IA) | ARP/AI se activa automáticamente por defecto en todos los volúmenes nuevos después de un periodo de gracia de 12 horas en las plataformas compatibles. | ONTAP 9.18.1 
...
| Tiempo de vida (TTL) de las credenciales positivas almacenadas en caché para las SVM de ONTAP | A partir de ONTAP 9.17.1, el tiempo de vida (TTL) de las credenciales válidas almacenadas en caché para las SVM de ONTAP se ha reducido de 24 horas (86 400 000 milisegundos) a 1 hora (3 600 000 milisegundos). | ONTAP 9.17.1 
...
| Cloud de SnapMirror | SnapMirror cloud aumenta el número de buckets S3 compatibles de 6 a 100. | ONTAP 9.19.1 


| Límite de control de flujo de la conexión S3 | El límite de control de flujo para conexiones ONTAP S3 ha disminuido de 2 MB a 256 KB para instancias cloud que tienen <= 32GB de memoria. | ONTAP 9.19.1 


| S3 tiempo de espera de conexión inactiva | El tiempo de espera de inactividad de la conexión S3 se ha reducido de 20 minutos a 5 minutos. | ONTAP 9.19.1 
...
| Emparejamiento de clústeres | Aumento del límite de pares de clúster admitidos de 255 a 400, lo que mejora la escalabilidad para entornos con un gran número de relaciones de pares de clúster. | ONTAP 9.14.1 
...
* El número máximo de miembros en un FlexGroup no puede exceder 24.
* Los límites de volumen de SVM-DR son de 500 por nodo y de 1000 por par de alta disponibilidad (incluidos los volúmenes de FlexVol y los componentes de FlexGroup).
+

NOTE: Los límites dependen de la plataforma. Debes comprobar cuáles son los límites correctos para tu configuración accediendo a link:https://hwu.netapp.com["NetApp Hardware Universe"^] y seleccionando tu plataforma y la versión de ONTAP.
...
| Transferencia simultánea de SnapMirror | El número máximo de transferencias simultáneas asíncronas a nivel de volumen ha aumentado de 100 a 200.  Las transferencias de SnapMirror Cloud han aumentado de 32 a 100 en sistemas de gama alta y de 6 a 20 transferencias de SnapMirror en sistemas de gama baja. | ONTAP 9,8 
