...
| xref:../smbc/index.html[SnapMirror Business Continuity (SM-BC) es compatible con SCIS3 reservas persistentes y clustering de conmutación al nodo de respaldo de Windows] | Las reservas persistentes de SCSI3 y la agrupación en clústeres de conmutación por error de ventanas para SMBC admiten múltiples nodos que acceden a un dispositivo, mientras que al mismo tiempo bloquean el acceso a otros nodos, lo que garantiza que la agrupación en clústeres para diferentes entornos de aplicación se mantenga consistente y estable. 


| xref:../data-protection/snapmirror-svm-replication-concept.html[Copiar copias Snapshot granulares de volúmenes con grupos de consistencia] | Se pueden utilizar grupos de coherencia para replicar snapshots de SnapMirror asíncronas y snapshots granulares de volúmenes en los grupos de coherencia de destino para una capa adicional de recuperación ante desastres. 
