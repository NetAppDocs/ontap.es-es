...
| link:../consistency-groups/protect-task.html#configure-asynchronous-snapmirror-protection["Compatibilidad de SnapMirror asíncrono con un único grupo de consistencia"] | Los grupos de coherencia son compatibles con configuraciones de SnapMirror asíncrono, lo que permite el almacenamiento de backups de SnapMirror para grupos de coherencia individuales. 
|===
