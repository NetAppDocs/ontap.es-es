...
| xref:../snaplock/set-retention-period-task.html[Capacidad de crear volúmenes de SnapLock y no de SnapLock en el mismo agregado] | A partir de ONTAP 9.10.1, pueden existir volúmenes SnapLock y no SnapLock en el mismo agregado, por lo que ya no es necesario crear un agregado de SnapLock independiente para volúmenes de SnapLock. 


| xref:../consistency-groups/index.html[Grupos de consistencia] | Volúmenes de clúster y LUN en grupos de coherencia para gestionar políticas y fidelidad en orden de escritura de volúmenes de almacenamiento para recuperación de snapshots. 
...
| xref:../flexcache/supported-unsupported-features-concept.html[Compatibilidad de NFSv4 con FlexCache] | Los volúmenes FlexCache admiten el protocolo NFSv4; artículo afectado en migración. 
