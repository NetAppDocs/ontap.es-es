...
* Conoce más sobre https://docs.netapp.com/us-en/ontap-afx/release-notes/whats-new-9191.html["Funciones de software ONTAP nuevas y mejoradas para los sistemas de almacenamiento NetApp AFX"^].
* Conoce más sobre https://docs.netapp.com/us-en/asa-r2/release-notes/whats-new-9191.html["Funciones de software ONTAP nuevas y mejoradas para sistemas de almacenamiento NetApp ASA r2"^].
...
== Aspectos destacados de ONTAP 9.19.1

ONTAP 9.19.1 ofrece funciones nuevas y mejoradas en los ámbitos de seguridad, escalabilidad y gestión del almacenamiento para apoyar la modernización de la infraestructura de datos. Estas mejoras te ayudan a escalar, operar de forma más eficiente y proteger los datos para el futuro. Para ver una lista completa de las nuevas funciones y mejoras, consulta xref:whats-new-9191.adoc[Novedades en ONTAP 9.19.1].

* xref:../performance-admin/guarantee-throughput-qos-task.html[Políticas de ráfagas de QoS configurables]
+
Las políticas de picos de QoS configurables permiten picos de rendimiento controlados y de duración limitada que superan los límites de QoS, lo que mejora la capacidad de respuesta de las cargas de trabajo durante los picos de actividad sin sacrificar la equidad ni requerir un sobredimensionamiento de los recursos.

* xref:../nfs-admin/enable-nfs-over-tls-task.html[NFS sobre TLS 1.3]
+
Protege los datos NFS en tránsito con cifrado TLS 1.3 basado en estándares, ayudando a reforzar la protección y a simplificar la adopción de NFS cifrado a escala empresarial. 

* xref:../s3-config/regenerate-access-keys.html[Varias claves de acceso a S3 por usuario]
+
Agrega compatibilidad con varias claves de acceso S3 activas por usuario, lo que permite una rotación fluida de credenciales sin tiempo de inactividad en la aplicación y ayuda a los clientes a reforzar la seguridad y el cumplimiento normativo mientras mantienen un acceso ininterrumpido a las cargas de trabajo de objetos.

* xref:../snapmirror-active-sync/protect-nas-task.html[SnapMirror active sync para NAS]
+
Gracias a la compatibilidad con NAS, la sincronización activa de SnapMirror ahora es una solución unificada de continuidad del negocio que ofrece un RPO de cero con conmutación por error transparente. 

* xref:../san-config/configure-fc-nvme-hosts-ha-pairs-reference.html[FC de conexión directa]
+
Incorpora compatibilidad con tarjetas HBA FC seguras, lo que permite una conectividad de host fiable y sienta las bases para una seguridad FC en tránsito respaldada por hardware, mientras se mantiene el rendimiento y se cumplen los requisitos de cumplimiento empresarial. 

* xref:../san-admin/multipath-automatic-lif-failover-support.html[Conmutación por error unificada active-active de LIF]
+
Los sistemas AFF ofrecen multivía activo-activo con conmutación automática por error de LIF, manteniendo todas las rutas SAN activas y optimizadas para que la E/S del host continúe de inmediato durante la conmutación por error. 

* link:https://docs.netapp.com/us-en/ontap-fli/get-started/learn-about-data-migration.html["Compatibilidad con iSCSI en FLI"^]
+
Agrega compatibilidad con iSCSI para la importación de LUN externos (FLI), lo que te permite migrar datos en bloques desde sistemas de almacenamiento de terceros a ONTAP utilizando la conectividad iSCSI existente con una interrupción mínima. 

* xref:../s3-config/create-modify-bucket-policy-task.html[Escrituras y eliminaciones condicionales en S3]
+
Permite el soporte de escrituras y eliminaciones condicionales con la API de S3, lo que permite a las aplicaciones aplicar comprobaciones de coherencia de datos durante las actualizaciones y eliminaciones de objetos, reduciendo el riesgo de sobrescrituras o eliminaciones accidentales en cargas de trabajo de objetos concurrentes y distribuidas.

* xref:../concepts/path-failover-concept.html#nas-path-failover[Conmutación por error combinada de LIF]
+
La conmutación automática por error de LIF es compatible con los sistemas AFF y ASA para las LIF del protocolo iSCSI en configuraciones multivía activo-activo y activo-activo-solo-SCSI, así como para las LIF del protocolo NVMe/TCP en configuraciones multivía activo-activo. 

* xref:../networking/improve-tcp-wan-performance.html[Rendimiento mejorado de la WAN]
+
El rendimiento mejorado de la WAN con TCP RACK mejora el rendimiento y la resiliencia en redes congestionadas, acelerando el acceso remoto a archivos y los servicios de datos entre sitios. 

* xref:../networking/configure-tls-offload.html[KTLS con aceleración por hardware para un cifrado más rápido del clúster de backend]
+
Mejora el cifrado del clúster de backend al introducir kTLS acelerado por hardware, descargando el procesamiento del cifrado a las SmartNIC compatibles para ofrecer mayor rendimiento y menor sobrecarga de CPU, mientras mantiene una comunicación segura y siempre activa en todo el tráfico del clúster y de alta disponibilidad (HA). 

* xref:../anti-ransomware/use-cases-restrictions-concept.html#snapmirror[Detección de ransomware basada en IA para cargas de trabajo replicadas de forma síncrona]
+
Amplía la protección autónoma contra el ransomware (ARP/AI) a SnapMirror synchronous synchronous para entornos SAN y NAS y a entornos SAN con SnapMirror active sync, preservando las señales de detección y recuperación en arquitecturas con RPO cero. 
...
+
Ofrece una interfaz gráfica de usuario (GUI) amigable para el inquilino para la gestión de SVM, eliminando la necesidad de una interfaz CLI compleja. 
...
== Aspectos destacados de ONTAP 9.18.1

ONTAP 9.18.1 ofrece características nuevas y mejoradas en las áreas de seguridad, escalabilidad y gestión de almacenamiento para respaldar la modernización de la infraestructura de datos.  Estas mejoras te ayudan a escalar, operar de manera más eficiente y proteger los datos para el futuro.  Para obtener una lista completa de las nuevas funciones y mejoras, consultexref:whats-new-9181.adoc[Novedades de ONTAP 9.18.1] .

* xref:../networking/configure-backend-encryption.html[Cifrado de alta disponibilidad del clúster de backend]
+
Capacidad para utilizar mTLS para cifrar la red del clúster de backend.

* xref:../networking/configure_network_security_using_federal_information_processing_standards_fips.html[Compatibilidad de PQC con datos en tránsito]
+
Capacidad para utilizar algoritmos post-cuánticos para el procesamiento de datos en tránsito.

* xref:../networking/ipsec-prepare.html[Compatibilidad con IPv6 IPSec con descarga de hardware]
+
Compatibilidad con IPSec con descarga de hardware con dirección IPv6.

* xref:../anti-ransomware/use-cases-restrictions-concept.html#supported-configurations[Compatibilidad con volúmenes FlexGroup de ARP/AI]
+
Los volúmenes NetApp FlexGroup admiten ARP/AI, lo que proporciona detección y protección avanzadas contra el ransomware basadas en aprendizaje automático.

* xref:../san-admin/nvme-copy-offload.html[Descarga de copia para NVMe]
+
Acelera las transferencias de grandes datos al descargar las operaciones de copia directamente al almacenamiento, reduciendo la carga de la CPU del host y de la red.

* xref:../s3-snapshots/list-restore-s3-snapshot.html[Restauraciones de instantáneas de buckets S3 controladas por almacenamiento]
+
...
+
ARP/AI es compatible con volúmenes SAN con detección de anomalías basada en cifrado, introduce nuevos comandos para obtener estadísticas detalladas de entropía y unifica los mensajes de protección contra ransomware en System Manager que antes se centraban en NAS. Los umbrales de detección configurables y una retención de instantáneas más determinista proporcionan mayor flexibilidad para diversas cargas de trabajo.
...
+
ONTAP 9.16.1 incorpora más comandos al marco MAV para ofrecer protección adicional frente a usuarios internos malintencionados. Estas mejoras incluyen muchos comandos de gestión de Consistency Group (CG), VScan y Autonomous Ransomware Protection (ARP/AI), y comandos de configuración de NVMe.

* xref:../anti-ransomware/index.html[Protección autónoma frente a ransomware con mejoras de la IA (ARP/AI)]
+
La protección autónoma contra el ransomware se ha actualizado con nuevas capacidades de inteligencia artificial, lo que le permite detectar y responder a los ataques de ransomware con una precisión y recuperación del 99 %. Como la IA está entrenada con un conjunto de datos exhaustivo, ya no hay un periodo de aprendizaje para ARP/AI ejecutándose en volúmenes FlexVol y ARP/AI comienza en modo activo de inmediato. ARP/AI también incorpora una función de actualización automática independiente de una actualización de ONTAP para garantizar protección y resiliencia constantes frente a las amenazas más recientes.
...
+
A partir de ONTAP 9.15.1, también tienes la opción de realizar una copia de seguridad del propio enlace simbólico en lugar de los datos a los que apunta. Esto puede aportar varias ventajas, entre ellas un mejor rendimiento de tus aplicaciones de backup. Puedes habilitar esta función mediante la CLI de ONTAP o la API de REST de ONTAP.
...
** xref:../anti-ransomware/support-arpai.html#multi-admin-verification-with-arp-ai-protected-volumes[Compatibilidad con verificación multiadministradora]
+
...
+
Si los componentes de un FlexGroup pierden el equilibrio, FlexGroup se puede reequilibrar y gestionar sin interrupciones desde la CLI de ONTAP, la API de REST de ONTAP y System Manager. Para un rendimiento óptimo, los miembros constituyentes dentro de un FlexGroup deben tener su capacidad utilizada distribuida de manera uniforme.
