...
[CAUTION]
====
* En las configuraciones de IP de MetroCluster, cuando ONTAP Mediator está configurado con el protocolo HTTPS, una única instancia de ONTAP Mediator solo puede supervisar una configuración de IP de MetroCluster. No se admite la supervisión simultánea de varias configuraciones de IP de MetroCluster.
...
** Responde `No` si planeas usar ONTAP Mediator solo para la sincronización activa de SnapMirror, o si planeas configurar ONTAP Mediator con HTTPS para monitorear una sola configuración IP de MetroCluster por instancia de ONTAP Mediator.
+
Alternativamente, puedes usar la opción `--https-only` en el comando `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y` para especificar solo HTTPS.
...
[NOTE]
====
Para omitir la comprobación de firmas durante la instalación, usa este comando: `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y --skip-code-signature-check`
