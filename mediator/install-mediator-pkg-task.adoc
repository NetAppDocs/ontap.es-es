...
summary: Para instalar o actualizar ONTAP Mediator, debes obtener el paquete de instalación y ejecutar el instalador en el host. 
---
...
* A partir de ONTAP 9.19.1 y ONTAP Mediator 1.12, tienes la opción de instalar ONTAP Mediator con compatibilidad con iSCSI y HTTPS, o compatibilidad solo con HTTPS. Si eliges instalar ONTAP Mediator con compatibilidad solo con HTTPS, el paquete SCST no se instala.
...
* Si tienes previsto utilizar ONTAP Mediator para supervisar varias configuraciones de IP de MetroCluster, la compatibilidad depende de tu tipo de Mediator, la versión de ONTAP y el sistema operativo. Consulta link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/concept_mediator_requirements.html#mediator-multiple-mccip["Utiliza ONTAP Mediator para supervisar varias configuraciones de MetroCluster IP"^] para más información.
...
** Tienes pensado configurar ONTAP Mediator con HTTPS para supervisar una o varias configuraciones de IP de MetroCluster.
...
* Durante la instalación o actualización a ONTAP Mediator 1.12 o posterior, el instalador te pedirá que respondas a lo siguiente:
+
...
+
** Responde `Yes` si planeas usar ONTAP Mediator configurado con iSCSI para monitorear una o más configuraciones IP de MetroCluster.
** Responde `No` si tienes previsto usar ONTAP Mediator solo para la sincronización activa de SnapMirror, o si planeas configurar ONTAP Mediator con HTTPS para monitorear una o más configuraciones de IP de MetroCluster.
+
Alternativamente, puedes usar la opción `--https-only` en el comando `./ontap-mediator-1.12.1/ontap-mediator-1.12.1` para especificar solo HTTPS.



* En los casos de uso de automatización, puedes usar el comando  `./ontap-mediator-1.12.1/ontap-mediator-1.12.1 -y` para saltarte las indicaciones del instalador.
...
[NOTE]
====
Para omitir la comprobación de firmas durante la instalación, usa este comando: `./ontap-mediator-1.12.1/ontap-mediator-1.12.1 --skip-code-signature-check`

====
+
El instalador crea las cuentas necesarias e instala los paquetes requeridos.  Si el Mediador ya está instalado, le solicitará que lo actualice.



.Ejemplo de instalación de ONTAP Mediator con soporte HTTPS e iSCSI (salida de consola)
[%collapsible]
====
[listing]
----
...
----
====
.Ejemplo de actualización de ONTAP Mediator con HTTPS y soporte iSCSI (salida de consola)
[%collapsible]
====
[listing]
----
...
----
====
.Ejemplo de instalación de ONTAP Mediator con soporte solo para HTTPS (salida de consola)
[%collapsible]
====
[listing]
----
...
----
====
.Ejemplo de actualización de ONTAP Mediator con soporte solo para HTTPS (salida de consola)
[%collapsible]
====
[listing]
----
...
== Registre la clave de seguridad para el arranque seguro UEFI.

A partir de ONTAP Mediator 1.4, el mecanismo de arranque seguro está habilitado en los sistemas UEFI.  Cuando el Arranque Seguro está habilitado, debe tomar pasos adicionales para registrar la clave de seguridad después de la instalación.


NOTE: Si instalaste ONTAP Mediator 1.12 o posterior con soporte solo para HTTPS, el paquete SCST no está instalado y puedes omitir esta tarea.
...
NOTE: Tras la instalación, la salida del sistema proporciona los archivos README y la ubicación de la clave.
...
[NOTE]
====
Puedes dejar la clave privada en su ubicación predeterminada o moverla a una ubicación segura.  Debes mantener la clave pública en su ubicación actual para que el Administrador de arranque pueda usarla.  Para obtener más información, consulte el archivo README.module-signing:

`[root@hostname ~]# ls /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/
README.module-signing  scst_module_key.der  scst_module_key.priv`

====
. Reinicie el host y utilice el Administrador de arranque UEFI de su dispositivo para aprobar el nuevo MOK.  Necesitarás la contraseña proporcionada para el `mokutil` utilidad enlink:index.html#install-ontap-mediator-when-uefi-secure-boot-is-enabled["Instale ONTAP Mediator cuando el inicio seguro de UEFI esté activado"] .




[[signing-scst-kernel-modules]]
== Firma de módulos del kernel SCST

Tras instalar ONTAP Mediator, si systemctl status `mediator-scst` Si aparece como fallido (inactivo), siga estos pasos para firmar el módulo del kernel SCST.


NOTE: Si instalaste ONTAP Mediator 1.12 o posterior con soporte solo para HTTPS, el paquete SCST no está instalado y puedes omitir esta tarea.

.Pasos
. Durante el proceso de compilación se genera un par de claves pública/privada en el `/opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/` directorio, utilizando el siguiente comando:
+
`[root@mediator-host ~]# ls /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/
README.module-signing  scst_module_key.der  scst_module_key.priv
[root@mediator-host ~]#`

. Inicie el proceso de importación de la clave pública al repositorio de claves UEFI ejecutando los siguientes comandos:
+
...
. El software mokutil solicita una contraseña temporal para esta clave durante el proceso de importación.
. Verifique si el proceso de importación ha comenzado con el `mokutil --list-new` y luego reinicie el sistema.  El gestor de arranque inicia el administrador EFI MOK.
. Utilice los menús en pantalla para activar la clave del módulo del kernel SCST.  Tras el arranque, ejecute `systemctl status mediator-scst` .  Una vez iniciado el servicio, se firman los módulos del kernel SCST.
