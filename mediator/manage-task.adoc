...
NOTE: Para los clústeres de continuidad de negocio de SnapMirror (SM-BC) y las configuraciones de MetroCluster IP que usan ONTAP Mediator configurado con HTTPS, debes añadir los certificados `intermediate.crt` y `root_ca.crt` a un clúster de ONTAP. Consulta link:../snapmirror-active-sync/mediator-install-task.html["Configurar ONTAP Mediator y los clústeres para la sincronización activa de SnapMirror"] y link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/task_configuring_the_ontap_mediator_service_from_a_metrocluster_ip_configuration.html["Configura el ONTAP Mediator para una configuración IP de MetroCluster"^].
