...
. Use la clave privada y el archivo de configuración para crear una solicitud de firma de certificación `ca.csr``:
+
*Ejemplo:*
...
NOTE: Para los clústeres de continuidad del negocio de SnapMirror (SM-BC), debe añadir el certificado `ca.crt` A un clúster de ONTAP. Consulte link:https://docs.netapp.com/us-en/ontap/smbc/smbc_install_confirm_ontap_cluster.html#ontap-mediator["Configure el mediador de ONTAP y los clústeres para SM-BC"].
...
.Pasos
. Sustituya el `ca.*` archivos con certificados de terceros.
...
.Pasos
. Configure el archivo `/opt/netapp/lib/ontap_mediator/ontap_mediator/server_config/ontap_mediator.user_config.yaml` para sustituir los valores de variable predeterminados en la `ontap_mediator.config.yaml` archivo.
+
...
----
Si se utiliza la validación del protocolo de estado de certificado en línea (OCSP), utilice el comando link:https://www.openssl.org/docs/manmaster/man1/openssl-verify.html^["openssl-verify"].
