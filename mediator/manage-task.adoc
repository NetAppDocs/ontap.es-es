...
Si la máquina virtual que ejecuta ONTAP Mediator necesita actualizarse con un kernel más nuevo, esto puede causar problemas de compatibilidad con los módulos del kernel SCST utilizados por ONTAP Mediator.  En este escenario, NetApp recomienda que desinstale y vuelva a instalar SCST manualmente.



=== Paso 1: Desinstalar SCST manualmente
...
=== Paso 2: Instalar SCST manualmente
...
NOTE: Si está realizando una instalación por primera vez y desea preinstalar SCST antes de instalar ONTAP Mediator, puede omitir este paso.  El instalador de ONTAP Mediator aplica cualquier parche relevante a los componentes SCST durante la instalación.
