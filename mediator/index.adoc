...
* Puedes usar cualquier versión de ONTAP Mediator para supervisar una configuración IP de MetroCluster.
...
Revisa estos puntos antes de actualizar o instalar ONTAP Mediator.
...
.Requisitos de red
Asegúrate de que los siguientes puertos están disponibles y no se usan para ONTAP Mediator:
...
* Si utilizas un cortafuegos de terceros, consulta link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/concept_mediator_requirements.html#firewall-requirements-for-ontap-mediator["Requisitos de firewall para ONTAP Mediator"^].
...
=== Consideraciones sobre la actualización del sistema operativo y compatibilidad del kernel
...
* No actualices el kernel más allá de la versión del sistema operativo compatible con tu versión de ONTAP Mediator, ya que es probable que el módulo SCST probado no funcione.
