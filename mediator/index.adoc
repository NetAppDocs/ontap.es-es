...
== Actualice el sistema operativo del host y, a continuación, el Mediator

La tabla siguiente proporciona las directrices de actualización si actualiza desde RHEL/CentOS 7.6 a una versión RHEL/CentOS posterior además de actualizar la versión de Mediator.

[cols="20,20,60"]
|===


| La versión de Linux de destino | Versión de Mediator de destino | Notas de actualización 
...
1.2
 a| 
* La actualización se debe realizar en el siguiente orden:
+
.. Actualice el sistema operativo desde la versión de RHEL/CentOS.
.. Reinicie el host para aplicar los cambios del módulo del kernel.
.. Actualice el Mediator desde la versión inmediatamente anterior a la versión actual.


* Para MetroCluster:
+
.. El comando Storage iscsi-initiator show informará de que la conexión con el servicio Mediator está inactiva durante la actualización.
.. El sistema operativo ONTAP generará los siguientes eventos de EMS:
+
... cf.mccip.med.auso.stDisabled durante la actualización
... cf.mccip.med.auso.stEnabled cuando se vuelve a habilitar la conmutación automática sin planificar
...
1.5
 a| 
. Actualice el sistema operativo desde la versión de RHEL/CentOS.
. Reinicie el host para aplicar los cambios del módulo del kernel. Si no reinicia el host, aparece un mensaje de error que le solicita que realice un reinicio.
. Actualice el Mediator desde la versión inmediatamente anterior a la versión actual.
...
| Todas las versiones RHEL/CentOS | Paquetes adicionales para RHEL/CentOS 7.x | Paquetes adicionales para RHEL 8.x 
...
Se recomienda contar con una certificación SSL válida, como se documenta en este procedimiento.
...
|===
Active el acceso a los repositorios enumerados anteriormente para que Mediator pueda acceder a los paquetes necesarios durante el proceso de instalación. Siga el procedimiento que se indica a continuación para el sistema operativo.

* Procedimiento para <<rhel7x,RHEL 7.x>> de netapp.
* Procedimiento para <<rhel8x,RHEL 8.x>> de netapp.
* Procedimiento para <<centos7x,CentOS 7.x>> de netapp.
...
Si su sistema operativo es *RHEL 7.x*:
...
Si su sistema operativo es *RHEL 8.x*:
...
Si su sistema operativo es *CentOS 7.x*:


NOTE: Los siguientes ejemplos muestran un repositorio para CentOS 7.6 y no puede funcionar para otras versiones de CentOS. Utilice el repositorio base para su versión de CentOS.

.Pasos
. Agregue el repositorio base C7.6.1810. El repositorio de almacén base C7.6.1810 contiene el paquete kernel-devel necesario para Mediador ONTAP.
...
. Descomprima el paquete del instalador: `tar xvfz ontap-mediator-1.5.0.tgz`
+
[listing]
----
...
* `ontap-mediator-1.5.0` | el ejecutable de instalación del producto para la versión 1.5.0
* `ontap-mediator-1.5.0.sig` | el SHA-256 hash, luego RSA-firmado usando la clave csc-prod, firma para el instalador
* `ontap-mediator-1.5.0.sig.tsr` | la solicitud de revocación para uso por parte de OCSCP para la firma del instalador
* `tsa-prod-ONTAP-Mediator.pem` | el certificado público del TSR
* `tsa-prod-chain-ONTAP-Mediator.pem` | la cadena de CA de certificado público para el TSR
...
+
.. Busque la URL de OCSP utilizada para registrar el certificado como certificados de desarrollador que podrían no proporcionar un uri.
+
[listing]
----
...
----
.. Conéctese al administrador de OCSP para enviar la solicitud OCSP: `openssl`
+
[listing]
----
...
. Verifique la cadena de confianza del CSC y las fechas de vencimiento con respecto al host local: `openssl verify`
+
...
----
. Compruebe el `ontap-mediator-1.5.0.sig.tsr` y.. `ontap-mediator-1.5.0.tsr` archivos que utilizan los certificados asociados: `openssl ts -verify`
+
...
----
. Verificar firmas con respecto a la clave:  `openssl -dgst -verify`
+
[listing]
----
...
+
** Siga las instrucciones del archivo README: `/opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/README.module-signing`Para firmar el módulo del kernel SCST.
** Localice las claves que desee: `/opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys`
...
.Pasos
. Ejecute el instalador y responda a las indicaciones según sea necesario: `./ontap-mediator-1.5.0/ontap-mediator-1.5.0 -y`
+
[listing]
----
...
.Ejemplo de instalación de ONTAP Mediator 1.5 (salida de consola)
[%collapsible]
====
[listing]
----
...
.Pasos
. Ejecute los siguientes comandos para ver el estado de los servicios Mediador de ONTAP:
+
.. Ejecución: `systemctl status ontap_mediator`
+
[listing]
----
...
----
.. Ejecución: `systemctl status mediator-scst`
+
[listing]
----
...
. Confirme los puertos que está utilizando el servicio Mediator de ONTAP: `netstat`
+
[listing]
----
...
El servicio Mediator de ONTAP ya está instalado y en ejecución. Es necesario llevar a cabo otra configuración en el sistema de almacenamiento de ONTAP para utilizar las funciones del mediador:
