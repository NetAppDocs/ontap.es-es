...
ONTAP Mediator depende de SCST para proporcionar iSCSI (consulta http://scst.sourceforge.net/index.html[]). Este paquete es un módulo del kernel que se compila durante la instalación específicamente para el kernel. Cualquier actualización del kernel podría requerir que reinstales SCST. Como alternativa, desinstala y vuelve a instalar ONTAP Mediator, y luego vuelve a configurar la relación de ONTAP.


NOTE: Cualquier actualización del kernel del SO servidor debe coordinarse con una ventana de mantenimiento en ONTAP.
