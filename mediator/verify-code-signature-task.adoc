...
[role="lead"]
Debe verificar la firma del código de Mediador de ONTAP antes de instalar el paquete de instalación de Mediador de ONTAP.
...
`ontap-mediator-1.8.0`
 a| 
Ejecutable de instalación del producto para la versión 1.8.0
...
----
. Compruebe el `ontap-mediator-1.8.0.sig.tsr` y.. `ontap-mediator-1.8.0.tsr` archivos que utilizan los certificados asociados:
+
`openssl ts -verify`
