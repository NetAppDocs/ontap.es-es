...
[role="lead"]
NetApp recomienda verificar la firma del código de Mediator de ONTAP antes de instalar el paquete de instalación de Mediator de ONTAP; sin embargo, este proceso es opcional.

.Antes de empezar
Antes de verificar la firma del código de ONTAP Mediator, el sistema debe cumplir los siguientes requisitos.


NOTE: Si el sistema no cumple con los siguientes requisitos, el proceso de verificación no es necesario y puede ir directamente a link:install-mediator-pkg-task.html["Instale el paquete de instalación del Mediador ONTAP"].
...
`ontap-mediator-1.9.0`
 a| 
Ejecutable de instalación del producto para la versión 1.9.0
...
----
. Verifique los `ontap-mediator-1.9.0.sig.tsr` archivos y `ontap-mediator-1.9.0.tsr` con los certificados asociados:
+
`openssl ts -verify`

+

NOTE: `.tsr` los archivos contienen la respuesta de marca de tiempo asociada con el instalador y la firma del código. El procesamiento confirma que la Marca de tiempo tiene una firma válida de TSA y que su archivo de entrada no ha cambiado.
La verificación se realiza de forma local en su máquina. Independientemente, no hay necesidad de acceder a los servidores TSA.
