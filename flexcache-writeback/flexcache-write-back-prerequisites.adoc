...
* El origen *_debe_* ejecutar ONTAP 9.15.1 o una versión posterior; sin embargo, ONTAP 9.15.1 no tiene todas las correcciones y mejoras necesarias para la escritura diferida de FlexCache, y no se recomienda para cargas de trabajo en producción. Se introdujeron mejoras significativas para la escritura diferida de FlexCache en ONTAP 9.17.1P1. Se *_recomienda encarecidamente_* que ejecutes la versión recomendada actual posterior a la 9.17.1P1 tanto en el origen como en los clústeres de caché. Si no puedes ejecutar ONTAP 9.17.1, la última versión P de 9.16.1 es la siguiente versión sugerida.
+

NOTE: Se recomienda *_encarecidamente_* que revises la link:flexcache-write-back-guidelines.html["Directrices de reescritura de FlexCache"] antes de implementar la escritura diferida de FlexCache en tu entorno.

* Cualquier clúster de caché que necesite funcionar en modo de escritura *_MUST_* ejecute ONTAP 9.15.1 o posterior.
* Cualquier clúster de almacenamiento en caché que no necesite funcionar en modo write-back puede ejecutar cualquier versión de ONTAP que admita volúmenes FlexCache.
