...
. En la ventana *Editar volumen*, localice la sección Configuración de instantáneas (local) y seleccione *Habilitar bloqueo de instantáneas*.
+
...
. Si el bloqueo de instantáneas aún no está activado, selecciona *Activar bloqueo de instantáneas*.
. Guarde los cambios.
