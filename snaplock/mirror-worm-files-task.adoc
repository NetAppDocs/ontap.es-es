...
---
= Refleje archivos WORM con ONTAP SnapMirror para recuperación ante desastres
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Una relación SnapMirror de un volumen SnapLock solo admite la `MirrorAllSnapshots` política de tipo async-mirror.  El período de retención de un volumen SnapLock está determinado por el período de retención máximo entre todos los archivos WORM que contiene.  Dado que el destino es una copia DR del origen, el período de retención del volumen SnapLock de destino será el mismo que el del origen.
...
NOTE: A partir de ONTAP 9.10.1, los volúmenes SnapLock y no SnapLock pueden existir en el mismo agregado; por lo tanto, ya no es necesario crear un agregado SnapLock separado si está usando ONTAP 9.10.1.  Utilice la opción de volumen -snaplock-type para especificar un tipo de volumen SnapLock de cumplimiento o empresarial.  En versiones de ONTAP anteriores a ONTAP 9.10.1, el modo SnapLock (Cumplimiento o Empresa) se hereda del agregado.  La configuración de idioma del volumen de destino debe coincidir con la configuración de idioma del volumen de origen.
