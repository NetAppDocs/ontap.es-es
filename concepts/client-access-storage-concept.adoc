...
---
= Conoce la autenticación de clientes y la autorización en ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
ONTAP admite el lenguaje de marcado de aserción de seguridad (SAML) para la autenticación de usuarios remotos. Se admiten varios proveedores de identidad populares (IdPs). Para obtener más información sobre IdPs admitidos e instrucciones para activar la autenticación SAML, consulta link:../system-admin/configure-saml-authentication-task.html["Configurar la autenticación SAML"].
...
La compatibilidad con el marco de autorización abierta (OAuth 2.0) está disponible a partir de ONTAP 9.14. Solo puedes usar OAuth 2.0 para tomar decisiones de autorización y control de acceso cuando el cliente utiliza la API de REST de ONTAP para acceder a ONTAP. Sin embargo, puedes configurar y habilitar la función con cualquiera de las interfaces administrativas de ONTAP, incluidas la CLI de ONTAP, System Manager y la API de REST de ONTAP.
