...
NOTE: A partir de ONTAP 9.15.1, puedes crear una reserva persistente para un volumen virtual usando SCSI-3. Esta función solo es compatible usando ONTAP tools for VMware vSphere 9 con un Windows Server Failover Cluster (WSFC).
...
VMware Live Site Recovery, anteriormente conocido como Site Recovery Manager (SRM), proporciona una capacidad de recuperación ante desastres. Live Site Recovery se integra con ONTAP tools for VMware vSphere para acceder y aprovechar la funcionalidad de gestión de datos de ONTAP.
