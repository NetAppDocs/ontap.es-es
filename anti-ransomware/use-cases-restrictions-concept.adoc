...
* El análisis de ARP/AI se ejecuta en el volumen activo de lectura y escritura en relaciones SAN sincrónicas de SnapMirror y de sincronización activa de SnapMirror. ARP crea instantáneas en el volumen principal y realiza la detección de ransomware basada en aprendizaje automático.
* Las instantáneas ARP no se replican en el volumen secundario en relaciones SnapMirror synchronous ni en relaciones SnapMirror active sync. Necesitas link:enable-task.html["habilitar ARP de nuevo"] en el volumen de recuperación después de la conmutación por error.
