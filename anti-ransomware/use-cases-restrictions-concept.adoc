...
* A partir de ONTAP 9.10.1 hasta 9.15.1, las cargas de trabajo NAS en volúmenes con más de 4,000 extensiones de archivo únicas.
+
El modelo ARP original puede analizar hasta 4,000 extensiones de archivo por volumen. Si el número de extensiones de archivo únicas en un volumen supera este límite, ARP no puede detectar de forma eficiente la actividad de ransomware y la carga de trabajo se considera inadecuada para ARP. Este límite no se aplica a ARP/AI a partir de ONTAP 9.16.1.
...
|===
^1^ Las versiones ONTAP 9.16.1 y 9.17.1 no admiten ARP/AI para los volúmenes FlexGroup; a partir de ONTAP 9.18.1, los volúmenes FlexGroup utilizan el modelo ARP/AI. Para conocer el comportamiento al actualizar a estas versiones, consulta <<arp-upgrade-considerations,Consideraciones sobre la actualización según la versión de ARP>>.

^2^ Consulta <<arp-upgrade-considerations,Consideraciones sobre la actualización según la versión de ARP>> para obtener información sobre el comportamiento predeterminado de la habilitación de ARP tras actualizar a ONTAP 9.18.1.

^3^ A partir de ONTAP 9.19.1, ARP/AI admite volúmenes primarios en relaciones síncronas de SnapMirror para NAS y SAN en FAS, AFF A-series, AFF C-series y sistemas AFX, y volúmenes primarios en relaciones SAN de sincronización activa SnapMirror en sistemas AFF A-series, AFF C-series y ASA r2. Para más información, consulta <<snapmirror>>.
...
* El estado de la configuración ARP y las instantáneas ARP no se replican en el volumen secundario como parte de SnapMirror synchronous o SnapMirror active sync en ONTAP 9.19.1.
...
== Consideraciones sobre la actualización según la versión de ARP

Al actualizar clústeres ONTAP que ya utilizan ARP, debes tener en cuenta los cambios de comportamiento específicos de ARP que se producen como consecuencia de la actualización, además de link:../upgrade/special-considerations.html["Aspectos especiales de ONTAP que debes resolver"].



[[arp-default-enablement-after-upgrading-to-ontap-9-18-1]]
=== Activación predeterminada de ARP tras actualizar a ONTAP 9.18.1

A partir de ONTAP 9.18.1, el comportamiento predeterminado de activación de ARP/AI está disponible para los sistemas AFF serie A y serie C, ASA y ASA r2. Tras una actualización, ARP/AI se activa automáticamente en todos los volúmenes nuevos tras un periodo de gracia de 12 horas. Debes link:index.html#manual-default-enablement["habilitar ARP manualmente"] en los volúmenes existentes.



[[snapmirror-relationships-after-upgrading-to-ontap-9-19-1]]
=== SnapMirror relaciones después de actualizar a ONTAP 9.19.1

Si habilitas ARP en volúmenes que forman parte de SnapMirror synchronous o de SnapMirror active sync relationships después de la actualización, la relación pasará brevemente a un estado de desincronización. Esta condición es temporal y la relación debería resincronizarse automáticamente sin intervención.



[[flexgroup-volumes-after-upgrade]]
=== FlexGroup volúmenes tras la actualización

ONTAP 9.16.1 y 9.17.1 no proporcionan soporte ARP/AI para volúmenes FlexGroup. Después de una actualización a estas versiones, los volúmenes FlexGroup habilitados para ARP siguen funcionando con el mismo modelo ARP usado antes de ARP/AI. A partir de ONTAP 9.18.1, los volúmenes FlexGroup usan el modelo ARP/AI.
