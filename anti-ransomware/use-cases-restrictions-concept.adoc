...
[role="lead"]
La protección autónoma de Ransomwware está disponible para cargas de trabajo NAS que comiencen con ONTAP 9.10.1. Antes de implementar ARP, debe tener en cuenta los usos recomendados y las configuraciones compatibles, así como las implicaciones de rendimiento.
...
| Volúmenes protegidos con SnapMirror asíncrono | X | X |  |  


| Los SVM protegidos con SnapMirror asíncrono | X | X |  |  


| Habilitar SVM para la migración de datos | X | X |  |  


| Volúmenes de FlexGroup | X |  |  |  


| Verificación de varios administradores | X |  |  |  
|===
...
* SnapMirror síncrono
* SnapMirror asíncrono (en ONTAP 9.10.1 a 9.12.1) Se admite SnapMirror asíncrono a partir de ONTAP 9.12.1. Para obtener más información, consulte <<snapmirror>>.)
