...
NOTE: Para reducir el elevado número de falsos positivos, ve a *Almacenamiento > Volúmenes > Seguridad > Configurar características de la carga de trabajo* y desactiva *Monitorizar nuevos tipos de archivos*. Esta configuración está desactivada por defecto en ONTAP 9.14.1 P7, 9.15.1 P1, 9.16.1 y posteriores. A partir de ONTAP 9.18.1 P2, System Manager ya no muestra las características de la carga de trabajo de ARP, incluidas las estadísticas de línea de base y de sobrecarga. Las estadísticas de sobrecarga siguen estando disponibles mediante las opciones de la CLI de ONTAP y la API de REST para análisis avanzados.
