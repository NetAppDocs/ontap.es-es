...
[role="lead"]
A partir de ONTAP 9.10.1, los administradores de ONTAP pueden activar Autonomous Ransomware Protection (ARP) para realizar análisis de cargas de trabajo en entornos NAS (NFS y SMB) para detectar y advertir de forma proactiva sobre actividades anómalas que puedan indicar un ataque de ransomware. ARP está integrado directamente en ONTAP, lo que garantiza control y coordinación integrados con las demás funciones de ONTAP. ARP funciona en tiempo real, procesando los datos a medida que se escriben o se leen del sistema de archivos y detectando y respondiendo rápidamente a posibles ataques de ransomware.

ARP crea instantáneas bloqueadas a intervalos regulares, además de las instantáneas programadas. La retención de instantáneas se ajusta dinámicamente: las instantáneas se reciclan rápidamente cuando no se detecta ninguna actividad inusual y las instantáneas tomadas antes de un ataque detectado se retienen durante más tiempo para investigación y recuperación.

Para conocer la compatibilidad del protocolo, la idoneidad de la carga de trabajo y los requisitos específicos de cada versión, consulta link:use-cases-restrictions-concept.html["Casos de uso y consideraciones sobre ARP"].

Para obtener más información sobre las instantáneas generadas por ARP, incluidos los cambios añadidos por versión de ONTAP, consulta <<Instantáneas de ARP>>.
...
A partir de ONTAP 9.18.1, ARP se habilita automáticamente por defecto en todos los volúmenes nuevos para AFF serie A y AFF serie C, ASA y ASA r2. Esta habilitación automática por defecto de ARP no se aplica a link:use-cases-restrictions-concept.html#unsupported-configurations["volúmenes o configuraciones no compatibles"]. Debes <<manual-default-enablement,habilitar ARP manualmente>> en los volúmenes existentes. Para detalles sobre cómo entra en vigor la habilitación predeterminada al actualizar, consulta link:use-cases-restrictions-concept.html#arp-upgrade-considerations["Consideraciones sobre la actualización según la versión de ARP"].


NOTE: A partir de ONTAP 9.19.1, la activación predeterminada de ARP se aplica a los clústeres que incluyen SnapMirror síncrono y relaciones SAN de sincronización activa SnapMirror. En ONTAP 9.18.1, la activación predeterminada se suprimió para estas configuraciones. Para las relaciones síncronas entre clústeres SnapMirror, la activación predeterminada se activa una vez que tanto el clúster de origen como el de destino se actualizan a ONTAP 9.19.1. ARP no está activado de forma predeterminada en los sistemas AFX, incluso con soporte síncrono de SnapMirror.

Durante el periodo de gracia, puedes link:default-enable-change.html["desactivar la activación por defecto para nuevos volúmenes a nivel de clúster usando System Manager o la ONTAP CLI"]. Si no te excluyes, ARP se habilita automáticamente para todos los nuevos volúmenes creados después del final del periodo de gracia. Si las necesidades cambian después del periodo de gracia, también tienes la flexibilidad de activar o desactivar la habilitación por defecto en cualquier momento.

.Activación manual predeterminada en nuevos volúmenes
Si desactivaste la activación automática por defecto de ARP a nivel de clúster, también puedes elegir link:enable-default-task.html["habilitar manualmente ARP de forma predeterminada en todos los volúmenes nuevos"] a nivel de SVM. Para ONTAP 9.17.1 y versiones anteriores, esta es la única forma de configurar ARP para que se active por defecto en los nuevos volúmenes.

[[manual-default-enablement]]
.Habilitación de ARP en todos o en volúmenes existentes específicos
A partir de 9.18.1, puedes habilitar manualmente ARP en todos los volúmenes existentes desde el nivel de clúster (selecciona *Cluster > Security* y image:icon_kabob.gif["Icono de opciones de menú"] en la sección *Anti-ransomware*, luego selecciona *Enable on all existing volumes*).

Si prefieres limitar la activación de ARP a un volumen específico, puedes link:enable-task.html["habilitar ARP por volumen"].
...
* link:https://docs.netapp.com/us-en/netapp-solutions-dataops/cyber-vault/ontap-cyber-vault-ps-hardening.html#ontap-cli-or-via-automation-script["Fortalecimiento de la bóveda cibernética de ONTAP con PowerShell"^]
...
Evaluación
 a| 
Se realiza un periodo de evaluación de dos a cuatro semanas para determinar el comportamiento de cifrado de referencia. La detección y las alertas pueden ocurrir mientras se establecen los umbrales de referencia. Puedes determinar si el periodo de evaluación está completo ejecutando el comando `security anti-ransomware volume show` y comprobando `Block device detection status`.

link:respond-san-entropy-eval-period.html["Conoce el periodo de evaluación de ONTAP ARP/AI para cargas de trabajo de dispositivos de bloque"].
 a| 
Volúmenes NAS que contienen discos virtuales de hipervisor detectados por ONTAP con ONTAP 9.17.1P5 y versiones posteriores
...
.Diferencias de instantáneas ARP por versión
* *Modificar la configuración de retención*: A partir de ONTAP 9.11.1, puedes link:modify-automatic-snapshot-options-task.html["modificar la configuración de retención de las instantáneas"].
* *Intervalos y respuesta ante anomalías*: A partir de ONTAP 9.17.1, las instantáneas ARP se generan a intervalos regulares tanto para volúmenes NAS como SAN, así como en respuesta a anomalías detectadas, que siempre ha sido la respuesta estándar. ONTAP antepone un nombre a la instantánea ARP para que sea fácilmente identificable.
* *SnapMirror synchronous y active sync*: A partir de ONTAP 9.19.1, las instantáneas ARP creadas para volúmenes que participan en relaciones de SnapMirror synchronous o SnapMirror active sync siguen estas reglas adicionales:
+
** Las instantáneas ARP se crean y conservan en el volumen primario.
** Las instantáneas ARP no se replican en el secundario como parte de SnapMirror synchronous o SnapMirror active sync. Necesitas link:enable-task.html["habilitar ARP de nuevo"] en el volumen de recuperación después de la conmutación por error.





NOTE: La restauración a nivel de volumen (`volume snapshot restore` en volúmenes SAN síncronos SnapMirror o de sincronización activa SnapMirror puede requerir que temporalmente pongas en pausa o interrumpas la relación síncrona SnapMirror o de sincronización activa SnapMirror. En muchos casos, puedes evitar interrumpir SnapMirror síncrono o de sincronización activa SnapMirror usando FlexClone u operaciones de restauración a nivel de archivo desde ARP u otras snapshots en el volumen primario.

link:use-cases-restrictions-concept.html#snapmirror["Más información sobre la compatibilidad con SnapMirror synchronous y SnapMirror active sync"].
