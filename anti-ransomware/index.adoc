...
[role="lead"]
A partir de ONTAP 9.10.1, los administradores de ONTAP pueden activar Autonomous Ransomware Protection (ARP) para realizar análisis de carga de trabajo en entornos NAS (NFS y SMB) para detectar y advertir proactivamente sobre actividad anómala que podría indicar un ataque de ransomware. A partir de ONTAP 9.17.1, ARP también es compatible con volúmenes de dispositivos de bloque, incluidos volúmenes SAN que contienen LUN o espacios de nombres NVMe, o volúmenes NAS que contienen discos virtuales de hipervisores como VMware. A partir de ONTAP 9.17.1P5, también se admiten los hipervisores Hyper-V, KVM y OpenStack.
