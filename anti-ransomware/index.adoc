...
Cuando la probabilidad de ataque sea baja, ONTAP creará una copia snapshot de tu volumen en segundo plano. La instantánea de APR se identifica por su nombre, por ejemplo `Anti_ransomware_backup.2022-12-20_1248`, y utiliza la etiqueta `anti-ransomware-backup`. Las amenazas que permanecen en el nivel bajo se registran, pero ONTAP no envía una alerta sobre amenazas bajas.

Cuando la probabilidad de ataque es moderada, ONTAP genera una notificación EMS que le solicita que evalúe la amenaza. Para obtener más información, consulte xref:respond-abnormal-task.html[Responda a actividades anormales].
