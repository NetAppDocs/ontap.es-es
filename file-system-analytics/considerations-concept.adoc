...
Al habilitar el análisis de capacidad, ONTAP realiza un escaneo de inicialización para dicho análisis. El escaneo accede a los metadatos de todos los archivos de los volúmenes en los que se ha habilitado el análisis de capacidad. Durante el escaneo no se leen datos de los archivos. A partir de ONTAP 9.14.1, puedes hacer un seguimiento del progreso del escaneo con la API de REST de ONTAP, en la pestaña **Explorer** de System Manager o con el comando de la CLI de ONTAP  `volume analytics show`. Si ocurre un evento de limitación, ONTAP te envía una notificación.
