...
[role="lead"]
A partir de ONTAP 9.12.1, ONTAP incluye un rol predefinido de control de acceso basado en roles (RBAC) llamado  `admin-no-fsa`. El rol  `admin-no-fsa` otorga privilegios de nivel de administrador, pero impide que el usuario realice operaciones relacionadas con el endpoint  `files` (es decir, File System Analytics) en la CLI de ONTAP, la API de REST de ONTAP y en System Manager.
