...
NOTE: `admin_SVM` es la SVM de administración del clúster. En los siguientes ejemplos, la SVM de administración se llama `cluster1`, por lo que `cluster1` aparece tanto en el símbolo del sistema (`cluster1::>`) como en el valor de `-vserver`.
