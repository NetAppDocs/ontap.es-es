...
summary: Puedes configurar el tiempo que ONTAP almacena las credenciales de los usuarios NFS en su caché interna modificando el servidor NFS de la máquina virtual de almacenamiento. Esto te permite aliviar ciertos problemas relacionados con la alta carga de los servidores de nombres o los cambios en las credenciales que afectan el acceso de los usuarios NFS. 
---
...
El TTL se mide en milisegundos. A partir de ONTAP 9.17.1 y versiones posteriores, el valor predeterminado es 1 hora (3,600,000 milisegundos). En ONTAP 9.16.1 y versiones anteriores, el valor predeterminado es 24 horas (86,400,000 milisegundos). El intervalo permitido para este valor es de 1 minuto (60,000 milisegundos) hasta 7 días (604,800,000 milisegundos).
