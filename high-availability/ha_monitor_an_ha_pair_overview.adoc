...
Debe establecer el valor como HA para que el nodo participe en una configuración de conmutación por error de almacenamiento (par de alta disponibilidad).
La `non-ha` el valor se utiliza solo en una configuración de clúster independiente o de nodo único. | `storage failover show -fields mode` 
