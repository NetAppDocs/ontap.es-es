...
A partir de ONTAP 9.18.1, puede configurar la comunicación cifrada entre nodos de pares HA. Para obtener más información, consultelink:ha-pair-encryption.html["Configure el cifrado para el tráfico de alta disponibilidad de ONTAP."] .
...
* *Monitorización y evitación de puertos*: en configuraciones de clúster de dos nodos sin switches, el sistema evita los puertos que experimentan pérdida total de paquetes (pérdida de conectividad). En ONTAP 9.8 y versiones anteriores, esta funcionalidad solo estaba disponible en configuraciones con switches.
