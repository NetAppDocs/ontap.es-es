...
NOTE: Si está realizando este paso en el primer nodo de un par de alta disponibilidad, debe marcar solo ese nodo como no elegible. No modifique el estado del socio de HA. Si el nodo asociado se selecciona como el nuevo maestro, debe verificar si contiene épsilon antes de declararlo no elegible. Esto se hace cuando repites estos pasos para eliminar el nodo asociado.
