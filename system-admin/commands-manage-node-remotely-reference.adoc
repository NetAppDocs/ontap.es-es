...
Más información sobre las opciones de comando `help` disponibles para SP y BMC en link:online-help-at-sp-bmc-cli-task.html["Consulte la información de ayuda en la CLI de ONTAP SP/BMC"].

Obtén más información sobre la CLI de BMC de ONTAP correspondiente en link:bmc-cli-commands-reference.html["Comandos de la CLI admitidos para el controlador de gestión de placa base de ONTAP"].
