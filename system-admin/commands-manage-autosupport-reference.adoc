...
---
= Comandos para gestionar AutoSupport en ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Utilice la `-force` Parámetro para enviar un mensaje incluso si AutoSupport está deshabilitado. Utilice la `-uri` parámetro para enviar el mensaje al destino que especifique en lugar del destino configurado.
...
|===
Obtenga más información sobre `system node autosupport` en el link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=system+node+autosupport["Referencia de comandos de la ONTAP"^].
