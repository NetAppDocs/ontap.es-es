...
Se puede acceder a las opciones de Nodeshell compatibles con clustershell mediante el uso de `vserver options clustershell` dominio. Para ver estas opciones, puede consultar la CLI de clustershell con `vserver options -vserver _nodename_or_clustername_ -option-name ?`
