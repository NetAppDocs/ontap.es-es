...
Cada nodo del clúster participa en un protocolo de votación que elige a un nodo como _primario_; el resto de nodos son _secundarios_. El nodo primario se encarga de sincronizar la información en todo el clúster. Cuando se forma el quórum, este se mantiene mediante votaciones continuas. Si el nodo primario se desconecta y el clúster sigue en quórum, los nodos que permanecen conectados eligen un nuevo nodo primario.
