...
* Puede especificar si SSLv3 debe utilizarse para el acceso seguro a la Web mediante `security config modify` con el `-supported-protocol` parámetro. De forma predeterminada, SSLv3 está deshabilitado. La seguridad de la capa de transporte 1.0 (TLSv1.0) está habilitada y se puede desactivar si es necesario.
...
====
+
** *Cuando el modo de cumplimiento FIPS 140-2 está desactivado* puede activar el modo de cumplimiento FIPS 140-2 configurando `is-fips-enabled` parámetro a. `true` para la `security config modify` y, a continuación, utilice el `security config show` comando para confirmar el estado en línea.
