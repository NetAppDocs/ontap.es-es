...
+
La `-application` parámetro de `security login` los comandos especifican el método de acceso para una cuenta de usuario. La `security login` las páginas de manual contienen información adicional.
...
* A partir de ONTAP 9.3, puede habilitar la autenticación multifactor en SSH para las cuentas de administrador locales.
+
...
* A partir de ONTAP 9.4, puede habilitar la autenticación multifactor SSH para usuarios remotos LDAP y NIS.
