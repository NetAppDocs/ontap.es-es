...
* A partir de ONTAP 9.13.1, puedes añadir opcionalmente la validación de certificados al proceso de autenticación SSH para mejorar la seguridad del inicio de sesión. Para hacer esto, link:../authentication/manage-ssh-public-keys-and-certificates.html["Asocie un certificado X,509 a la clave pública"] que usa una cuenta. Si inicias sesión usando SSH con una clave pública SSH y un certificado X.509, ONTAP verifica la validez del certificado X.509 antes de autenticar con la clave pública SSH. El inicio de sesión SSH se rechaza si ese certificado está caducado o revocado, y la clave pública SSH se desactiva automáticamente.
* A partir de ONTAP 9.14.1, los administradores de ONTAP pueden link:../authentication/configure-cisco-duo-mfa-task.html["Agregue la autenticación de dos factores Cisco Duo al proceso de autenticación SSH"] para mejorar la seguridad del inicio de sesión. En el primer inicio de sesión después de activar la autenticación de Cisco Duo, los usuarios tendrán que inscribir un dispositivo para que sirva como sistema de autenticación para las sesiones SSH.
* A partir de ONTAP 9.15.1, los administradores pueden link:../authentication/dynamic-authorization-overview.html["Configure la autorización dinámica"] para proporcionar autenticación adaptativa adicional a los usuarios de SSH en función de la puntuación de confianza del usuario.
* A partir de ONTAP 9.19.1, los administradores pueden usar el link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=security+ssh+publickey+show["Comando security ssh publickey show -vserver  -host-key-algorithm "^] para ver la huella digital de la clave del host de ONTAP. Esto permite a los administradores verificar que esta huella digital coincide con la que está almacenada en ONTAP.
