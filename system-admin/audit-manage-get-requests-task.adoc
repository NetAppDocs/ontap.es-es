...
summary: Mientras que las peticiones SET se registran por defecto, las peticiones GET no. Sin embargo, puedes utilizar el comando settings para controlar si las peticiones get enviadas desde ONTAP HTML, la línea de comandos de ONTAP o desde las APIs de ONTAP se registran en el archivo. 
---
...
[role="lead"]
Mientras que las peticiones SET se registran por defecto y no se pueden desactivar, las peticiones GET no se registran por defecto. Sin embargo, puedes controlar si las peticiones GET enviadas desde ONTAP HTML (`-httpget`), la ONTAP CLI (`-cliget`) o desde las ONTAP APIs (`-ontapiget`) se registran en el archivo.
