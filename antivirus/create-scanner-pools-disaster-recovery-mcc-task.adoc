...
* Para los pools de análisis definidos para una SVM individual, debe haber configurado el conector antivirus ONTAP con la LIF de gestión de SVM o la LIF de datos de SVM.
* Para los pools de análisis definidos para todas las SVM de un clúster, debe haber configurado el conector antivirus de ONTAP con la LIF de gestión del clúster.
...
Esto significa que debe crear pools de análisis primarios y secundarios en cada clúster de una configuración de MetroCluster que correspondan a las SVM primaria y secundaria en el clúster. El pool secundario pasa a estar activo cuando el clúster comienza a servir datos de la SVM secundaria. En la siguiente ilustración, se muestra una configuración de MetroCluster típica.

image::../media/metrocluster-av-config.gif[MetroCluster av config]
...
\u1,cifs\u2
----
. Compruebe que se han creado los grupos de escáneres: `vserver vscan scanner-pool show -vserver _data_SVM|cluster_admin_SVM_ -scanner-pool _scanner_pool_`
+
Para obtener una lista completa de las opciones, consulte la página de manual del comando.
