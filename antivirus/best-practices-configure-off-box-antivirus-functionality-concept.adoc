...
* El valor de  `scan-queue-timeout` debe ser menor que el valor de  `scanner-pool request-timeout`. Si lo configuras en un valor más alto, el acceso a los archivos podría retrasarse y, eventualmente, agotarse el tiempo de espera. Para evitar esto, configura el  `scan-queue-timeout` en 5 segundos menos que el valor de  `scanner-pool request-timeout`. Consulta la documentación del proveedor del motor de análisis para ver cómo cambiar la configuración de  `scan-queue-timeout`. El  `scanner-pool timeout` se puede cambiar usando el siguiente comando en modo avanzado y proporcionando el valor adecuado para el parámetro  `request-timeout`:
