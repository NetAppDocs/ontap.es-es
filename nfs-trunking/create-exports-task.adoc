...
Requisitos de exportación de ONTAP:

* La política de exportación por defecto del volumen raíz de SVM debe incluir una regla que permita a todos los clientes el acceso abierto a través de NFS.
+
Más información sobre link:../nfs-config/open-export-policy-svm-root-volume-task.html["Acceso de cliente de NFS en la SVM de ONTAP."]

* Debes agregar la misma regla a la política por defecto si creaste una política de exportación personalizada.
+
Más información sobre link:../nfs-admin/default-export-policy-svms-concept.html["políticas de exportación predeterminadas para los SVM NFS de ONTAP."]
