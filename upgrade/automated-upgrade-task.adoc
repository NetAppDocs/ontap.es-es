...
* Si no tiene previsto supervisar el progreso del proceso de actualización, debería hacerlo link:task_requesting_notification_of_issues_encountered_in_nondisruptive_upgrades.html["Solicite notificaciones EMS de errores que puedan requerir intervención manual"].
...
. Si va a actualizar una configuración de MetroCluster FC, compruebe que el clúster esté habilitado para una conmutación automática no planificada.
+

NOTE: Si va a actualizar una configuración estándar o una configuración IP de MetroCluster, no es necesario realizar este paso.
