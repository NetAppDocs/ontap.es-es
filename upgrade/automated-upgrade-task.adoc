...
---
= Realiza una actualización no disruptiva automatizada de ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Cuando realizas una actualización no disruptiva automatizada (ANDU), ONTAP instala automáticamente la imagen de ONTAP de destino en cada nodo, valida que el clúster se pueda actualizar correctamente y luego ejecuta una xref:concept_upgrade_methods.html[actualización por lotes o sucesivas] en segundo plano según la cantidad de nodos en el clúster.
...
NOTE: Si está actualizando a ONTAP 9.15.1 o posterior a través de la consola de NetApp , siga laslink:https://docs.netapp.com/us-en/console-software-updates/get-started/software-updates.html["procedimiento de actualización en la documentación de la consola de NetApp"^] .


IMPORTANT: Modificar la configuración de la opción de comando  `storage failover modify-auto-giveback` antes de que comience una ANDU no afecta el proceso de actualización. El proceso ANDU ignora cualquier valor preestablecido para esta opción durante la toma de control o devolución requeridas para la actualización. Por ejemplo, establecer  `-autogiveback` en false antes de iniciar la ANDU no interrumpe la actualización automática antes de la devolución. Obtén más información sobre  `storage failover modify-auto-giveback` en link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=storage+failover+modify-auto-giveback["Referencia de comandos del ONTAP"^].
...
* Si lo prefieres, link:../update/pre-stage-system-firmware.html["preparar el firmware del sistema para una actualización con un solo reinicio"].
+
A partir de ONTAP 9.17.1, si ya hay una versión más reciente del BIOS o del firmware del LOADER en el dispositivo de arranque de un nodo antes de realizar un ANDU, el proceso de actualización conserva el firmware más reciente en lugar de sobrescribirlo con la versión incluida en la imagen de ONTAP. Esto permite que tanto la actualización del software ONTAP como la del firmware se completen en un único ciclo de reinicio.
...
+
Para FTP anónimo, debes especificar la URL en el formato \ftp://anonymous@ftpserver.
...
|===
.. Selecciona *Validar* para ejecutar las comprobaciones de validación previas a la actualización.
+
...
+
A partir de ONTAP 9.19.1, los errores y las advertencias se muestran por separado para ayudarte a identificar fácilmente los problemas que deben resolverse antes de actualizar.
...
+
Se muestra una lista de las comprobaciones previas completas de la actualización automatizada junto con los errores o advertencias que deben solucionarse antes de iniciar el proceso de actualización. Debes resolver todos los errores antes de proceder con la actualización. También es buena práctica resolver las advertencias.
...
====
.El futuro
Si has preparado previamente el firmware, utiliza el comando  `system node firmware bootmedia show` para comprobar que la versión de firmware prevista está activa en cada nodo.
