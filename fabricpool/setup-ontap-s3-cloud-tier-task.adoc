...
== Configura el nivel de cloud ONTAP S3 para FabricPool
...
== Cambia el FQDN de ONTAP S3 para el tiering de FabricPool

Cuando tengas que realizar cambios en tu red que incluyan actualizaciones del FQDN, debes cambiar el FQDN de ONTAP S3 que se utiliza para el tiering de FabricPool para evitar interrupciones en la red.

.Pasos
. Desactiva la verificación de certificados para el nivel FabricPool:
+
`storage aggregate object-store config modify -object-store-name <object store> -is-certificate-validation-enabled false`

. Cambia el FQDN del servidor ONTAP S3:
+
`vserver object-store-server modify -vserver <vserver> -object-store-server <new fqdn>`

. (Opcional) Si es necesario validar el certificado y se ha cambiado el nombre de host, link:../s3-config/create-install-ca-certificate-svm-task.html["renovar el certificado"].
. Actualiza el nivel FabricPool con el nuevo FQDN:
+
`storage aggregate object-store config modify -object-store-name <object_store_name> -server <new fqdn>`

. Verifica que todos los almacenes de objetos, y sus réplicas si las hay, estén disponibles en el nivel FabricPool:
+
`storage aggregate object-store show -fields object-store-availability,mirror-type,is-mirror-degraded`

+
Para obtener más información, consulta link:https://docs.netapp.com/us-en/ontap-cli/storage-aggregate-object-store-show.html#description["almacenamiento agregado objeto-almacén mostrar"^].

. Repite estos pasos en cualquier almacén de objetos adicional.
. Si renovaste los certificados en el servidor ONTAP S3 y renovaste los certificados en el nivel FabricPool, vuelve a habilitar la verificación de certificados:
+
