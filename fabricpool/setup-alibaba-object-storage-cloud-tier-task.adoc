...
+
Los sistemas AFF recientemente solicitados incluyen 10 TB de capacidad libre para usar FabricPool. Si necesita capacidad adicional en un sistema AFF, si usa el almacenamiento de objetos en la nube de Alibaba en un sistema que no sea AFF, o si actualiza desde un clúster existente, necesita una link:../fabricpool/install-license-aws-azure-ibm-task.html["Licencia de FabricPool"].
