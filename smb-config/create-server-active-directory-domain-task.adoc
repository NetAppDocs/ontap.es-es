...
Necesitas los siguientes privilegios mínimos en la unidad organizativa (OU) para crear el servidor SMB:
...
* Escritura validada en service principal names
* Lee msDS-SupportedEncryptedTypes
* Escribe msDS-SupportedEncryptedTypes
...
* Puedes elegir añadir el servidor SMB a una OU diferente usando la opción  `-ou`.
* A partir de ONTAP 9.13.1, puedes configurar un sitio predeterminado para el servidor SMB. ONTAP puede usar este sitio cuando una LIF no tiene pertenencia a ningún sitio y como alternativa si falla la detección de sitios para un dominio de confianza al localizar los controladores de dominio de dominios de confianza.
...
. Crear el servidor SMB en un dominio de AD: `+vserver cifs create -vserver vserver_name -cifs-server smb_server_name -domain FQDN [-ou organizational_unit][-default-site site_name][-netbios-aliases NetBIOS_name, ...][-keytab-uri {(ftp|http)://hostname|IP_address}][-comment text]+`
+
...
A partir de ONTAP 9.13.1, el sitio predeterminado del servidor SMB también se usa como alternativa si ONTAP no puede detectar la información del sitio de un dominio de confianza.
