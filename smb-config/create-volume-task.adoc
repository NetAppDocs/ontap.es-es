...
.Pasos
. Cree el volumen con un punto de unión: `volume create -vserver vserver_name -volume volume_name -aggregate aggregate_name -size {integer[KB|MB|GB|TB|PB]} -security-style ntfs -junction-path junction_path]`
+
...
. Compruebe que el volumen se ha creado con el punto de unión deseado: `volume show -vserver vserver_name -volume volume_name -junction`
