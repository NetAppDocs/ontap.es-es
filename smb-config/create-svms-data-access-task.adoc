...
.Pasos
. Cree una SVM: `vserver create -vserver vserver_name -rootvolume root_volume_name -aggregate aggregate_name -rootvolume-security-style ntfs -language C.UTF-8 -ipspace ipspace_name`
+
...
. Compruebe la configuración y el estado de la SVM recién creada: `vserver show -vserver vserver_name`
+
