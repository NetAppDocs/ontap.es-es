...
+
Después de la inicialización del par HA (arranque), la asignación automática de propiedad del disco se habilita automáticamente y utiliza la política de medio cajón para asignar propiedad a las unidades restantes (excepto las unidades contenedoras que tienen la partición raíz) y cualquier unidad agregada en el futuro, como reemplazar unidades fallidas, responder a un mensaje de "repuestos bajos" o agregar capacidad.
