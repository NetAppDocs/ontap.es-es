...
+
`-p pattern1 -p pattern2 -p pattern3` especifica un ciclo de uno a tres bytes hexadecimales definidos por el usuario
patrones de sobrescritura que se pueden aplicar sucesivamente a los discos que se están saneando. El valor predeterminado
el patrón es de tres pasadas, usando 0x55 para la primera pasada, 0xaa para la segunda pasada, y 0x3c para la
tercera pasada.
