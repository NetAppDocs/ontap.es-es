...
---
= No se puede llegar al mediador o el estado de quórum del mediador es falso
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Consulte link:smbc_install_initialize_the_ontap_mediator.html#["Inicialice el Mediador ONTAP"].
...
.Solución:
Si Mediator no está configurado, debe configurar Mediator antes de poder establecer una relación SM-BC. Solucione cualquier problema de conectividad de red. Asegúrese de que Mediator está conectado y que el estado de quórum es verdadero en el sitio de origen y de destino mediante el comando snapmirror mediador show.
