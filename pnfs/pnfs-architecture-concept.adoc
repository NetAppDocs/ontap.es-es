...
NOTE: En estos comandos no aparecen los nombres de los volúmenes. En su lugar, se utilizan los identificadores numéricos asociados a dichos volúmenes: el MSID y el DSID. Para encontrar los volúmenes asociados a las asignaciones, puedes usar  `volume show -dsid [dsid_numeric]` o  `volume show -msid [msid_numeric]` en privilegio avanzado de la CLI de ONTAP.
...
* https://www.netapp.com/pdf.html?item=/media/12385-tr4571.pdf["Informe técnico 4571 de NetApp : Prácticas recomendadas de FlexGroup"^]
