...
[role="lead"]
A partir de ONTAP 9.9.1, puedes usar _SnapMirror cloud_ cuando quieras trasladar tus flujos de trabajo de protección de datos a la nube. En lugar de las arquitecturas heredadas de copia de seguridad en cinta, puedes usar el almacenamiento de objetos como repositorio alternativo para la retención y el archivado de datos a largo plazo. Puedes usar proveedores como Amazon S3, Azure Blob, Google Cloud Storage, StorageGRID u ONTAP S3 como tu almacén de objetos en la nube.
...
+
** Cuando utilices más de 6 buckets por clúster, deberías usar la configuración de URL de estilo virtual-hosted para los proveedores que la admitan, incluidos AWS S3, Alibaba Cloud, StorageGrid, Google Cloud Storage, IBM Cloud object storage, ONTAP S3 y otros proveedores compatibles con S3. Se recomienda esta configuración para permitir el direccionamiento escalable de almacén de objetos y evitar las limitaciones subyacentes.
