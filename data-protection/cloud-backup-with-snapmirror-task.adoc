...
---
= Haz una copia de seguridad de los datos en la nube usando ONTAP SnapMirror cloud
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
_SnapMirror cloud_ ofrece replicación ONTAP a destinos de almacén de objetos. Esto te permite sustituir las arquitecturas tradicionales de copia de seguridad en cinta por almacenamiento de objetos para la conservación y el archivado de datos a largo plazo. Puedes usar proveedores como Amazon S3, Alibaba Cloud, Azure Blob, Google Cloud Storage, IBM Cloud Object Storage, NetApp StorageGRID u ONTAP S3 como tu almacén de objetos en la nube.
...
* SnapMirror cloud aumenta el número de buckets de almacén de objetos S3 admitidos por clúster de 6 a 100. Este límite se aplica al número total de almacenes de objetos configurados en todos los puntos finales de almacén de objetos SnapMirror cloud en un clúster ONTAP. Cuando uses esta función, ten en cuenta las siguientes consideraciones:
+
...
* SnapMirror cloud admite operaciones de copia de seguridad y restauración para volúmenes FlexGroup en configuraciones MetroCluster usando el link:https://docs.netapp.com/us-en/ontap-automation/get-started/access_rest_api.html["API de REST de ONTAP"^] existente. Esta funcionalidad te permite crear relaciones de SnapMirror cloud para volúmenes FlexGroup en configuraciones MetroCluster que se mantienen desde el sitio asociado después de una conmutación de sitios y switchback.


Antes de usar la función en la nube de SnapMirror, debes solicitar una clave de licencia de la API de SnapMirror en el sitio de soporte de NetApp: link:https://mysupport.netapp.com/site/tools/snapmirror-cloud-api-key["Solicite una clave de licencia de la API de cloud de SnapMirror"^].
...
NOTE: Es posible que necesites tanto una clave de licencia de SnapMirror Cloud como una clave API. Aunque ONTAP One incluye la clave de licencia de SnapMirror Cloud, se requiere la clave de licencia para habilitar la función de SnapMirror Cloud. Es posible que debas generar e instalar la clave de licencia por separado.
