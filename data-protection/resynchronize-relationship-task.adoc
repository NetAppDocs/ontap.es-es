...
* A partir de ONTAP 9.13.1, ONTAP intenta utilizar la resincronización rápida de forma predeterminada para reducir el tiempo de resincronización. Para que la resincronización rápida se utilice de forma predeterminada, deben cumplirse las siguientes condiciones:
+
** Los volúmenes FlexVol no tienen clones en el volumen.
** Al usar la política MirrorAllSnapshots
+

NOTE: Usando `-quick-resync` puede consumir espacio adicional en el volumen de destino de resincronización debido a la pérdida de eficiencia de almacenamiento en los bloques de datos transferidos. Este consumo de espacio adicional se recupera como parte de la aplicación de eficiencia de almacenamiento en línea o posterior a la replicación en el destino de resincronización.

+
El `-quick-resync` El parámetro es opcional. Puede activar o desactivar la resincronización rápida mediante el siguiente método: `-quick-resync true|false` parámetro con el `snapmirror resync` dominio.

+
Para obtener más información sobre `-quick-resync`, ver el link:https://docs.netapp.com/us-en/ontap-cli/snapmirror-resync.html["Referencia de comandos del ONTAP"^].
...
* link:resync-svm-destination-task.html["Vuelva a sincronizar los datos en una SVM de destino de ONTAP SnapMirror"]
