...
| Replicación de snapshots creada por aplicaciones | ONTAP 9,7 | Si una instantánea se etiqueta con la etiqueta adecuada en el momento de la  `snapshot create` operación, usando la CLI o la API de ONTAP, SnapMirror síncrono replica las instantáneas, tanto las creadas por el usuario como las creadas con scripts externos, después de poner en estado de espera las aplicaciones. A partir de ONTAP 9.14.1, ya no existe el requisito de la etiqueta para la replicación de instantáneas creadas por aplicaciones. Para más información sobre la replicación de instantáneas creadas por aplicaciones, consulta el link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/SnapMirror/How_to_replicate_application_created_snapshots_with_SnapMirror_Synchronous["Base de conocimientos de NetApp : Cómo replicar snapshots creados por aplicaciones con SnapMirror sincrónico"^]. 


| Protección autónoma contra ransomware | ONTAP 9.19.1 |  
...
| Restauración de NDMP/NDMP | ONTAP 9.13.1 | Tanto el clúster de origen como el de destino deben ejecutar ONTAP 9.13.1 o una versión posterior para poder realizar copias de seguridad NDMP basadas en dump o en SMTape de volúmenes síncronos de SnapMirror. Las restauraciones NDMP, ya sean basadas en dump o en SMTape, no son compatibles con volúmenes de destino en una relación de SnapMirror active sync o en una relación síncrona de SnapMirror. 
...
| Replicación de las operaciones de gestión de instantáneas | ONTAP 9.19.1 | A partir de ONTAP 9.19.1, las operaciones de gestión de instantáneas `snapshot delete`, `snapshot rename` y `snapshot modify` se replican en el clúster secundario. 


| Replicación programada de instantáneas | ONTAP 9.19.1 | A partir de ONTAP 9.19.1, puedes activar la replicación de instantáneas programadas usando una política de SnapMirror. En versiones anteriores a ONTAP 9.19.1, las instantáneas programadas creadas usando una política de instantáneas no se replican. 
...
| Recuperación ante desastres de SVM | ONTAP 9,6  a| 
* Una fuente síncrona de SnapMirror también puede ser una fuente de recuperación ante desastres de SVM, por ejemplo, una configuración en abanico con SnapMirror síncrono como una rama y recuperación ante desastres de SVM como la otra.
* Una fuente síncrona de SnapMirror no puede ser un destino de recuperación ante desastres de SVM porque SnapMirror síncrono no admite la cascada de una fuente de protección de datos. Debes liberar la relación síncrona antes de realizar una resincronización de flip de recuperación ante desastres de SVM en el clúster de destino.
* Un destino síncrono de SnapMirror no puede ser una fuente de recuperación ante desastres de SVM, ya que la recuperación ante desastres de SVM no admite la replicación de volúmenes DP. Una resincronización tras el cambio de la fuente síncrona provocaría que la recuperación ante desastres de SVM excluyera el volumen DP del clúster de destino.




| link:../snaplock/snapshot-lock-concept.html["Snapshots a prueba de manipulación"] | ONTAP 9.19.1 | Puedes especificar el tiempo de caducidad solo durante la creación de instantáneas. No puedes establecer ni ampliar el tiempo de caducidad en instantáneas existentes. 
...
* Restauración NDMP basada en volcado o SMTape en volúmenes de destino en una relación de sincronización activa de SnapMirror o una relación sincrónica de SnapMirror
...
== Replicación programada de instantáneas

A partir de ONTAP 9.19.1, puedes usar el parámetro de política de SnapMirror `-sync-snapshot-replication-mode` en modo de privilegio avanzado para replicar snapshots programadas al destino de SnapMirror. Replicar snapshots programadas proporciona simetría entre los volúmenes primario y secundario en una relación síncrona de SnapsMirror.

.Requisitos previos
* Debes ser administrador de clúster o SVM
* Debes estar en modo de privilegios avanzados
* Debes configurar una política SnapMirror definida por el usuario que use el parámetro `-sync-snapshot-replication-mode` e incluya la opción `sched_created`.
+
** Las políticas definidas por el sistema no se pueden modificar. Debes crear una nueva política y restablecer la relación.
** Los cambios de política definidos por el usuario requieren una resincronización (quiesce seguida de resume) para que surtan efecto en las relaciones existentes.




El parámetro acepta estas opciones:

* `none`- desactiva la replicación de instantáneas síncronas que no son SnapMirror
* `application_created`- replica las instantáneas creadas por la aplicación
* `sched_created` - replica las instantáneas programadas
* `sm_created` - replica instantáneas creadas por SnapMirror asíncrono (fanout leg)
* Una lista de tipos separada por comas: `application_created`, `sched_created`, y `sm_created`.


El parámetro solo tiene efecto una vez que el estado de la relación es `InSync`. Los tipos de instantáneas que no aparecen en la lista solo se almacenan en el volumen primario.

A partir de ONTAP 9.19.1, `snapmirror initialize` replica todas las instantáneas en el clúster secundario, mientras que `snapmirror resync` solo replica las nuevas instantáneas creadas después de la última instantánea común. Durante una operación de resincronización, las instantáneas no replicadas etiquetadas como solo primarias cuando el estado de la relación es `InSync` no se replican en el clúster secundario, y esas instantáneas se conservan en el nuevo clúster secundario tras una operación de resincronización inversa.


NOTE: La creación de instantáneas en volúmenes síncronos SnapMirror puede aumentar la latencia de E/S del usuario. Se recomienda utilizar planificaciones de instantáneas con una frecuencia superior a 30 minutos.

.Información relacionada
* https://www.netapp.com/pdf.html?item=/media/17174-tr4733.pdf["Informe técnico de NetApp 4733: Prácticas recomendadas y configuración síncrona de SnapMirror"^]
* link:https://docs.netapp.com/us-en/ontap-cli//volume-snapshot-create.html["crear instantánea de volumen"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-delete.html["eliminar snapshot de volumen"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-modify.html["modificar instantánea de volumen"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-rename.html["renombrar instantánea de volumen"^]
