...
* A partir de ONTAP 9.19.1, no se admite la activación de IPsec en SVMs configurados para SVM DR. Si usas IPsec, debes desactivarlo o elegir un SVM diferente para configurar una relación SVM DR. Recibirás un error si usas ONTAP 9.19.1 o una versión posterior e intentas habilitar IPsec en un SVM configurado para SVM DR.
...
. Desde el clúster de origen, crea una relación entre iguales SVM usando el comando  `vserver peer create`.
+
