...
Tanto las puestas en marcha ramificadas como aquellas en cascada son compatibles con cualquier combinación de recuperación de desastres de SnapMirror, SnapVault o replicación unificada. Sin embargo, las relaciones síncronas de SnapMirror (compatibles a partir de ONTAP 9.5) solo son compatibles con puestas en marcha ramificadas que suponen una o más relaciones asíncronas de SnapMirror y no son compatibles con las puestas en marcha en cascada. Solo una relación en la configuración ramificada puede ser una relación síncrona de SnapMirror; todas las demás relaciones del volumen de origen deben ser relaciones asíncronas de SnapMirror. xref:../snapmirror-active-sync/recover-unplanned-failover-task.html[SnapMirror síncrono activo] (Compatible a partir de ONTAP 9.3,1) también admite configuraciones de distribución ramificada.
