...
+
** Puedes añadir uno o varios eventos separados por comas a la política FPolicy.
...
+
El siguiente comando crea una política llamada  `policy1` que tiene el evento llamado  `event1` y el motor externo llamado  `engine1` asociados a ella. Esta política usa valores predeterminados en la configuración de la política:
`vserver fpolicy policy create -vserver vs1.example.com -policy-name policy1 -events event1 -engine engine1`

+
El siguiente comando crea una política llamada  `policy2` que tiene el evento llamado  `event2` y el motor externo llamado  `engine2` asociados a ella. Esta política está configurada para usar acceso privilegiado con el nombre de usuario especificado. La lectura directa está habilitada:
...
+
El siguiente comando crea una política denominada  `native1` que tiene el evento denominado  `event3` asociado. Esta política usa el motor nativo y usa los valores predeterminados en la configuración de la política:
