...
summary: Pueden surgir problemas cuando no hay espacio suficiente ni en los volúmenes de almacenamiento temporal ni en el volumen que contiene los registros de eventos de auditoría. Si no hay espacio suficiente, no se pueden crear nuevos registros de auditoría. Cuando la garantía de auditoría está habilitada, esto puede impedir que los clientes accedan a los datos y provocar que las solicitudes de acceso fallen. Debes saber cómo diagnosticar y resolver estos problemas de espacio en los volúmenes. 
---
...
[role="lead"]
Pueden surgir problemas cuando no hay espacio suficiente ni en los volúmenes de almacenamiento temporal ni en el volumen que contiene los registros de eventos de auditoría. Si no hay espacio suficiente, no se pueden crear nuevos registros de auditoría. Cuando la garantía de auditoría está habilitada, esto puede impedir que los clientes accedan a los datos y provocar que las solicitudes de acceso fallen. Debes saber cómo diagnosticar y resolver estos problemas de espacio en los volúmenes.

[NOTE]
====
Este comportamiento se aplica cuando la garantía de auditoría está activada. Para obtener más información, consulta link:../nas-audit/auditing-process-concept.html["Auditoría garantizada"].
...
Si los volúmenes que contienen archivos de registro de eventos se quedan sin espacio, la auditoría no puede convertir los registros de eventos en archivos de registro. Cuando la garantía de auditoría está habilitada, esto provoca errores de acceso de los clientes. Debes saber cómo solucionar los problemas de espacio relacionados con los volúmenes de registro de eventos.
...
Si alguno de los volúmenes que contienen archivos de almacenamiento temporal para tu storage virtual machine (SVM) se queda sin espacio, la auditoría no puede escribir registros de log en los archivos de almacenamiento temporal. Cuando la garantía de auditoría está habilitada, esto provoca fallos de acceso de los clientes. Para solucionar este problema, necesitas determinar si alguno de los volúmenes de almacenamiento temporal usados en la SVM está lleno mostrando información sobre el uso del volumen.
