...
---
= Protege con ONTAP SnapMirror active sync para SAN
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Configurar la protección asimétrica para configuraciones SAN usando SnapMirror active sync implica seleccionar los LUN en el clúster de origen de ONTAP y agregarlos a un grupo de consistencia.
...
* El clúster de origen y el clúster de destino no pueden formar parte de una configuración de MetroCluster.
...
Puedes configurar la protección simétrica para configuraciones SAN mediante System Manager o la CLI de ONTAP. En ambas interfaces, hay pasos diferentes para xref:index.html#key-concepts[configuraciones uniformes y no uniformes].
...
. En el sitio primario, link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes["crear un grupo de consistencia utilizando nuevos LUNs"].
+
...
.System Manager
--
. En el sitio primario, link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes["crear un grupo de coherencia utilizando nuevos volúmenes o espacios de nombres NVMe"].
