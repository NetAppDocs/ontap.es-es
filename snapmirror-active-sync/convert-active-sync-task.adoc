...
---
= Convierte una relación SAN existente de ONTAP SnapMirror en una relación SAN de SnapMirror active sync
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Si has configurado la protección de SnapMirror, puedes convertir una relación SAN en SnapMirror active sync para SAN. A partir de ONTAP 9.15.1, puedes convertir la relación para usar protección simétrica active/active.

Para obtener información sobre cómo convertir una relación SVM-DR en una relación de sincronización activa SnapMirror para NAS, consulta link:protect-nas-task.html#convert-an-svm-dr-relationship-to-a-snapmirror-active-sync-relationship["Convierte una relación SVM DR en una relación de sincronización activa de SnapMirror"].
...
.Antes de empezar
* Debe existir una relación sincrónica SnapMirror de RPO cero entre el clúster primario y el secundario.
...
* link:mediator-install-task.html["Mediador ONTAP"] Debe configurarse para la sincronización activa de SnapMirror .
