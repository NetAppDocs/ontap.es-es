...
A-Series y C-Series
 a| 
Dúplex con conmutación automática al respaldo (asimétrico activo/activo), conmutación automática al respaldo (asimétrico activo/pasivo)
 a| 
* 2 (ONTAP 9.9.1 o posterior)
* 4 (ONTAP 9.16.1 con configuraciones simétricas activo/activo)
...
A-Series y C-Series
 a| 
Dúplex con conmutación automática al respaldo (simétrico activo/activo), conmutación automática al respaldo (asimétrico activo/pasivo)
 a| 
* 2 (ONTAP 9.9.1 o posterior
* 4 (ONTAP 9.16.1 con configuraciones simétricas activo/activo)
...
Todo
 a| 
Dúplex con conmutación automática al respaldo (simétrico activo/activo)
 a| 
* 2 (ONTAP 9.17.1 o anterior)
* 4 (ONTAP 9.18.1 o posterior)
...
A-Series y C-Series
 a| 
Asimétrico activo/pasivo (conmutación automática al respaldo)
 a| 
2 (ONTAP 9.19.1 o posterior)
...
Asimétrico activo/pasivo (conmutación automática al respaldo)
 a| 
4 (ONTAP 9.19.1 o posterior)

|===
.Replicación entre plataformas
...
NOTE: Si tus sistemas de almacenamiento ONTAP se adquirieron antes de junio de 2019, consulta link:https://mysupport.netapp.com/site/systems/master-license-keys["Claves de licencia de ONTAP"^] para obtener la licencia sincrónica de SnapMirror necesaria.
...
SnapMirror active sync es compatible con los siguientes protocolos SAN.
...
** Configuraciones activas/pasivas asimétricas
** Cambios en el tamaño del grupo de consistencia
+
No puedes ampliar ni reducir un grupo de coherencia de forma no disruptiva cuando usas el protocolo NVMe con SnapMirror active sync. Las operaciones de ampliación y reducción de grupos de coherencia son disruptivas cuando usas el protocolo NVMe con SnapMirror active sync.
...
A partir de ONTAP 9.19.1, la sincronización activa de SnapMirror es compatible con los siguientes protocolos NAS.

* NFSv3 y posteriores
+
Se admite la conmutación por error sin interrupciones para NFSv3 y versiones posteriores.

* SMB
+
** Se admite la conmutación por error sin interrupciones para los recursos compartidos SMB 3 CA.
** Se admite la conmutación por error disruptiva (pero manteniendo un RPO de cero) para recursos compartidos SMB2 y SMB3 no CA.
** SMB1 no es compatible.




Los clústeres NAS requieren la misma configuración de dirección IP y protocolo en el clúster primario y en el secundario para permitir una conmutación por error sin interrupciones. Dado que los SVM pueden configurarse con la misma dirección IP y los mismos protocolos, SnapMirror active sync admite configuraciones NAS a nivel de SVM; sin embargo, el SVM secundario permanece inactivo (no se ejecuta) porque las LIF deben permanecer desactivadas hasta que se produzca una conmutación por error para evitar colisiones de IP con el SVM primario.
...
Con la sincronización activa de SnapMirror para NAS, los SVM primarios y secundarios pueden estar en diferentes IPspaces.
...
El estilo de seguridad NTFS *no* es compatible en los volúmenes de SnapMirror active sync para SAN. El estilo de seguridad NTFS es compatible con SnapMirror active sync para NAS.
...
* En versiones anteriores a ONTAP 9.15.1, las relaciones de sincronización activa de SnapMirror no son compatibles en volúmenes de destino de lectura-escritura (volúmenes convertidos a lectura-escritura desde DP en una configuración activa-pasiva asimétrica). Antes de que puedas usar un volumen de lectura-escritura, debes convertirlo en un volumen DP creando una relación de SnapMirror a nivel de volumen (ya sea asíncrona o síncrona) y luego eliminando la relación. Para más detalles, consulta link:convert-active-sync-task.html["Convierta unas relaciones de SnapMirror existentes en sincronización activa de SnapMirror"].
