...
A-Series y C-Series
 a| 
Conmutación automática por error dúplex (activo/activo simétrico), conmutación automática por error (activo/pasivo asimétrico)
 a| 
...
NOTE: Si sus sistemas de almacenamiento ONTAP se adquirieron antes de junio de 2019, consulte link:https://mysupport.netapp.com/site/systems/master-license-keys["Claves de licencia maestra de ONTAP"^] para obtener la licencia sincrónica de SnapMirror requerida.
...
* En versiones anteriores a ONTAP 9.15.1, las relaciones de sincronización activa de SnapMirror no son compatibles con volúmenes de destino de lectura/escritura (volúmenes convertidos a lectura/escritura desde DP en un activo-pasivo asimétrico). Antes de que puedas usar un volumen de lectura/escritura, tienes que convertirlo en un volumen DP creando una relación SnapMirror a nivel de volumen (ya sea asíncrona o síncrona) y luego eliminando la relación. Para más detalles, consulta link:convert-active-sync-task.html["Convierta unas relaciones de SnapMirror existentes en sincronización activa de SnapMirror"].
