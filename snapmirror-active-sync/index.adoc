...
[role="lead"]
SnapMirror Active Sync (lanzado inicialmente como SnapMirror Business Continuity [SM-BC]) permite que los servicios empresariales continúen funcionando incluso si se produce un fallo total del sitio; así, las aplicaciones pueden recuperarse tras fallos de forma transparente usando una copia secundaria. Se requiere intervención manual ni secuencias de comandos personalizadas para activar una recuperación tras fallos con SnapMirror: Sincronización activa.

La sincronización activa de SnapMirror está disponible a partir de ONTAP 9,9.1. La sincronización activa de SnapMirror se admite en clústeres AFF, clústeres de cabinas All Flash SAN (ASA) y C-Series (AFF o ASA). Los clústeres primario y secundario deben tener del mismo tipo: ASA o AFF. La sincronización activa de SnapMirror protege las aplicaciones con LUN iSCSI o FCP.
...
NOTE: A partir de julio de 2024, el contenido de informes técnicos publicados anteriormente como archivos PDF se ha integrado con la documentación de los productos de ONTAP. La documentación de sincronización activa de SnapMirror de ONTAP ahora incluye contenido de TR-4878: Sincronización activa de SnapMirror.
...
* Capacidad de alojar aplicaciones críticas alternativamente desde sitio primario y secundario.
...
* Grupos de consistencia (que es la unidad de conmutación por error); para cada grupo de consistencia, se proporciona la siguiente información:
+
...
Al crear la protección con _simétrico activo/activo_, el proceso depende de la configuración:
