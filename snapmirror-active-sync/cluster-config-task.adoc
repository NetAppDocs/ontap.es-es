...
. Se crea al menos un LIF de SAN (FC o iSCSI, según corresponda) para la configuración de SAN en cada nodo de ambos clústeres, o al menos un LIF de NAS (NFS o SMB, según corresponda) se crea en cada nodo del clúster primario para la replicación al clúster secundario.
+
...
+
link:../nfs-config/index.html["Conoce la configuración de NFS"]

+
link:../smb-config/index.html["Conoce la configuración de SMB"]

. Solo SAN: Se crean los LUN necesarios y se asignan a un igroup, que se utiliza para asignar los LUN al iniciador del host de la aplicación.
+
link:../san-admin/provision-storage.html["Cree LUN y asigne iGroups"]

. Se vuelve a analizar el host de la aplicación para detectar cualquier LUN o volumen nuevo.
