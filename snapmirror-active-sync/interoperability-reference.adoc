...
Para obtener más información sobre la compatibilidad de las configuraciones NAS, consulta link:protect-nas-task.html["Protege con la sincronización activa de SnapMirror de ONTAP para NAS"].
...
Un evento de conmutación automática no planificada (AUFO) en el clúster primario aislado puede deberse a un fallo de doble evento cuando se pierde la conexión entre el clúster primario y el secundario, y también se pierde la conexión entre el clúster primario y el mediador. Esto se considera un evento poco frecuente, a diferencia de otros eventos AUFO.
...
A partir de ONTAP 9.19.1, ARP/AI admite volúmenes primarios que participan en relaciones SAN de sincronización activa de SnapMirror. Para obtener más información, consulta link:../anti-ransomware/use-cases-restrictions-concept.html#interoperability-and-vm-considerations["Casos de uso y consideraciones sobre la protección de Ransomware autónoma"].
...
SnapMirror active sync para SAN es compatible con xref:../data-protection/supported-deployment-config-concept.html[configuraciones de dispersión] con la política  `MirrorAllSnapshots` y, a partir de ONTAP 9.11.1, con la política  `MirrorAndVault`. Las configuraciones fan-out no son compatibles en SnapMirror active sync con la política  `XDPDefault`.

A partir de ONTAP 9.15.1, la sincronización activa de SnapMirror admite la reconfiguración automática en el tramo de distribución tras un evento de conmutación por error. Si la conmutación por error del sitio primario al secundario se ha realizado con éxito, la reconfiguración de la relación de distribución se inicia durante la siguiente actualización programada del tramo de distribución asíncrono. No se activa inmediatamente después de la conmutación por error de la sincronización activa de SnapMirror. El tramo de distribución asíncrona puede ser una relación de grupo de consistencia en una configuración SAN o una relación de volumen independiente. La reconfiguración funcionará en cualquiera de los dos casos. La reconfiguración se activa tanto en una conmutación por error planificada como no planificada. La reconfiguración también se produce al volver al sitio primario.
...
A partir de ONTAP 9.12.1, link:../data-protection/restore-part-file-snapshot-task.html["Restauración parcial de LUN"] es compatible para los volúmenes de sincronización activa de SnapMirror. Puedes restaurar datos desde instantáneas creadas por aplicaciones que se hayan replicado entre el origen de sincronización activa de SnapMirror (volumen) y los volúmenes de destino (instantáneas). Puede ser necesario restaurar parcialmente un LUN o un archivo si necesitas restaurar una base de datos en un host que almacena varias bases de datos en el mismo LUN. Para usar esta funcionalidad, necesitas conocer el desplazamiento inicial en bytes de los datos y la cantidad de bytes.
