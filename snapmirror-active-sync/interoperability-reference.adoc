...
A partir de ONTAP 9.19.1, la conmutación automática al respaldo transparente de aplicaciones para AIX es compatible con SnapMirror active sync en configuraciones activas/activas simétricas. Puedes usar System Manager o la CLI de ONTAP para crear un LUN de AIX. Para garantizar la conmutación automática al respaldo transparente, cuando uses la CLI de ONTAP, debes usar la opción access-mode local con el comando lun create o lun modify. Por defecto, el modo de acceso está establecido en global. Cuando uses System Manager, elige AIX como host y local como modo de acceso. Para obtener más información sobre la creación de un LUN, consulta link:../san-admin/provision-storage.html["Aprovisione el almacenamiento SAN"].
