...
Los límites de los grupos de consistencia para un clúster con una configuración SAN de sincronización activa SnapMirror se calculan en función de las relaciones y dependen de la versión de ONTAP utilizada. Los límites son independientes de la plataforma.
...
El número máximo de volúmenes por grupo de consistencia con SnapMirror active sync para configuraciones SAN no depende de la plataforma.
...
Los límites de volumen en la sincronización activa de SnapMirror para configuraciones SAN se calculan en función del número de endpoints, no del número de relaciones. Un grupo de consistencia con 12 volúmenes aporta 12 endpoints tanto en el clúster primario como en el secundario. Tanto la sincronización activa de SnapMirror como las relaciones síncronas de SnapMirror contribuyen al número total de endpoints.


IMPORTANT: Estos límites se aplican a los sistemas AFF y ASA. Si tienes un sistema ASA r2 (ASA A1K, ASA A90, ASA A70, ASA A50, ASA A30 o ASA A20), consulta link:https://docs.netapp.com/us-en/asa-r2/data-protection/manage-consistency-groups.html["Documentación de ASA r2"^].
...
| Número de LUN, espacios de nombres o unidades de almacenamiento únicos por cada solución de sincronización activa SnapMirror de 4 x 4 (disponible a partir de ONTAP 9.16.1) | 6.144 
...
[[nas-object-limits]]
== Límites de los objetos NAS

A partir de ONTAP 9.19.1, SnapMirror active sync es compatible con los protocolos NAS. Los límites de objetos NAS se incluyen en la siguiente tabla.

|===
| Máximo de objetos en una relación de sincronización activa de SnapMirror | Cuente 


| Número de unidades de conmutación por error por par de alta disponibilidad | 50 


| Número de SVM de NAS por par de HA | 50 


| Número de volúmenes por SVM | 80 


| Número de grupos de consistencia de aplicaciones por par de alta disponibilidad | No admitido 


| Número de volúmenes por grupo de consistencia | No aplicable 


| Número de volúmenes por par de HA | 400 


| Pares de clústeres que puede gestionar un único mediador | 10 


| Número de nodos en un clúster  a| 
2 (solo clústeres AFF)

4 (solo clústeres AFX)
