...
Es posible que quieras realizar una conmutación por error planificada para comprobar el estado de tu configuración de recuperación ante desastres o para hacer mantenimiento en el clúster principal.
...
. Para supervisar el estado de la conmutación por error, utiliza el comando  `snapmirror failover show` en la CLI de ONTAP.
...
+
Ejemplo de conmutación al respaldo en una configuración SAN:

+
destination::>snapmirror failover start -destination-path vs1_dst:/cg/cg_dst

+
Ejemplo de conmutación al respaldo en una configuración de NAS:

+
destination::>snapmirror failover start -destination-path vs_b:

. Desde el destino, supervisa el progreso de la conmutación automática al respaldo:
+
