...
Cuando se utiliza ONTAP Mediator para supervisar una relación de sincronización activa de SnapMirror, ONTAP Mediator no se puede inicializar en ONTAP sin un certificado válido autofirmado o de entidad emisora de certificados (CA). Agrega un certificado válido al almacén de certificados para clústeres peered.
