...
A partir de ONTAP 9.19.1 y ONTAP Mediator 1.12, puedes usar la misma instancia de ONTAP Mediator para supervisar simultáneamente una configuración IP de MetroCluster de cuatro nodos y una relación de paridad entre clústeres de SnapMirror active sync. Tu configuración debe cumplir con los siguientes requisitos:

* ONTAP Mediator 1.12.1 o una versión posterior está instalado en un host con una CPU de 2 núcleos.
* La instancia de ONTAP Mediator está configurada con el protocolo HTTPS.
+

NOTE: Solo puedes supervisar una configuración IP de cuatro nodos de MetroCluster y una relación de sincronización activa entre iguales de SnapMirror usando la misma instancia de ONTAP Mediator.
...
IMPORTANT: Si estás usando SnapMirror active sync y ONTAP Mediator o ONTAP Cloud Mediator con ONTAP 9.17.1, deberías revisar los *Problemas y limitaciones conocidos* en el link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2492508["Notas de la versión de ONTAP"^] para información importante sobre estas configuraciones.
...
Cuando se utiliza ONTAP Mediator para supervisar una relación de sincronización activa SnapMirror, ONTAP Mediator no se puede inicializar en ONTAP sin un certificado autofirmado válido o un certificado de autoridad de certificación (CA). Añades un certificado válido al almacén de certificados para clústeres peered.
...
-----END CERTIFICATE-----
----
. Instala los certificados de CA raíz e intermedia en el clúster primario o secundario:
+
...
+
Introduce `root.crt` cuando se te solicite, como se muestra en el siguiente ejemplo:
...
----
. Repite el paso anterior para instalar los certificados de CA raíz e intermedio en el clúster de iguales.
. Ver los certificados CA autofirmados instalados utilizando el nombre generado del certificado:
+
