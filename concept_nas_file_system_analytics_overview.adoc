...
Con FSA, puede ver todos los niveles de la jerarquía de sistemas de archivos de un volumen en NAS. Por ejemplo, puede obtener información sobre uso y capacidad en los niveles de máquina virtual de almacenamiento (SVM), volumen, directorio y archivo. Puede utilizar la FSA para responder preguntas como:
...
image:flexgroup1.png["análisis de FlexGroup"]
...
Cada lanzamiento de ONTAP amplía el alcance analítico del análisis del sistema de archivos.
...
| Visualización en System Manager | X | X | X | X | X 


| Análisis de capacidad | X | X | X | X | X 


| Información de datos inactivos | X | X | X | X | X 


| Compatibilidad con volúmenes que han realizado la transición desde Data ONTAP 7-Mode | X | X | X | X |  


| Capacidad para personalizar el período inactivo en System Manager | X | X | X | X |  


| Seguimiento de actividad a nivel de volumen | X | X | X |  |  


| Descargue los datos de seguimiento de actividad en CSV | X | X | X |  |  


| Seguimiento de actividad a nivel de SVM | X | X |  |  |  
