...
summary: La creación de una política de auditoría para máquinas virtuales de almacenamiento es el tercer paso en la configuración y aplicación de ACLs a un archivo o carpeta. Una política actúa como un contenedor para varias tareas, donde cada tarea es una única entrada que se puede aplicar a archivos o carpetas. Puedes añadir tareas a la política de seguridad más adelante. 
---
