...
summary: Bypass traverse checking es un derecho de usuario que determina si un usuario puede atravesar todos los directorios de la ruta a un archivo, incluso si el usuario no tiene permisos en el directorio atravesado. Debes entender qué sucede cuando permites o no el bypass traverse checking y cómo configurar el bypass traverse checking para los usuarios en las máquinas virtuales de almacenamiento. 
---
...
* xref:assign-privileges-concept.html[Lista de privilegios compatibles]
