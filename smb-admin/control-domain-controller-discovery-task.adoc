...
summary: A partir de ONTAP 9.3, puedes modificar el proceso predeterminado por el que se descubren los controladores de dominio. Esto te permite limitar la detección a tu sitio o a un grupo de DC preferidos, lo que puede mejorar el rendimiento según el entorno. 
---
...
+
El `default-site` parámetro del servidor SMB se puede definir para usar este modo con LIF que no estén asignadas a ningún sitio en sites-and-services. A partir de ONTAP 9.13.1, el sitio predeterminado también se puede usar como alternativa si ONTAP no puede detectar la información del sitio para un dominio de confianza.
