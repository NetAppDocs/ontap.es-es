...
summary: Antes de que los usuarios puedan crear conexiones SMB para acceder a los datos contenidos en la SVM, deben ser autenticados por el dominio al que pertenece el servidor SMB. El servidor SMB admite dos métodos de autenticación, Kerberos y NTLM. Kerberos es el método predeterminado utilizado para autenticar a los usuarios del dominio. 
---
