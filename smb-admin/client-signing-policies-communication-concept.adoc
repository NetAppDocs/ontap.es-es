...
[NOTE]
====
Microsoft ya no recomienda usar `Digitally sign communications (if client agrees)` o `Digitally sign communications (if server agrees)` la configuración de directiva de grupo. Microsoft tampoco recomienda usar la configuración de registro `EnableSecuritySignature`. Estas opciones solo afectan el comportamiento de SMB 1 y pueden reemplazarse por la configuración de directiva de grupo `Digitally sign communications (always)` o la configuración de registro `RequireSecuritySignature`. También puedes obtener más información en link:http://blogs.technet.com/b/josebda/archive/2010/12/01/the-basics-of-smb-signing-covering-both-smb1-and-smb2.aspx["Blog de Microsoft: los conceptos básicos de la firma SMB (cubriendo tanto SMB1 como SMB2)"^].
