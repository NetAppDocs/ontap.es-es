...
summary: Para aprovechar la máxima seguridad con la comunicación basada en Kerberos, puedes activar el cifrado AES-256 y AES-128 en el servidor SMB. Si no quieres que el servidor SMB seleccione los tipos de cifrado AES para la comunicación basada en Kerberos con el KDC de Active Directory, puedes desactivar el cifrado AES. Por defecto, el cifrado AES está desactivado. 
---
...
Cuando modificas el parámetro  `-advertised-enc-types`, los tipos de cifrado especificados reemplazan cualquier tipo configurado previamente. Se eliminan todas las configuraciones anteriores de tipos de cifrado.

Para mejorar la seguridad, la máquina virtual de almacenamiento (SVM) cambia la contraseña de su cuenta de máquina en el AD cada vez que se modifica la opción de seguridad AES. Cambiar la contraseña podría requerir credenciales administrativas de AD para la unidad organizativa (OU) que contiene la cuenta de máquina. Solo aparece una solicitud de credenciales cuando ONTAP debe restablecer la contraseña de la cuenta de máquina en Active Directory y tu configuración de AD requiere credenciales administrativas para la unidad organizativa que contiene la cuenta de máquina. Si tu AD está sincronizado y el restablecimiento automático está habilitado, no aparece ninguna solicitud.
