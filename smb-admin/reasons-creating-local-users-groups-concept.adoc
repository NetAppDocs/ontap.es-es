...
summary: Hay varias razones para crear usuarios locales y grupos locales en tu máquina virtual de almacenamiento. Por ejemplo, puedes acceder a un servidor SMB usando una cuenta de usuario local si los controladores de dominio no están disponibles, podrías querer usar grupos locales para asignar privilegios o tu servidor SMB está en un grupo de trabajo. 
---
