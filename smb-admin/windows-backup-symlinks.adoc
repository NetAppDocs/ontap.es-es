...
summary: Cuando una aplicación de backup que se ejecuta en Windows encuentra un enlace simbólico de estilo Unix, se sigue el enlace y se realiza la copia de seguridad de los datos. A partir de ONTAP 9.15.1, tienes la opción de realizar copias de seguridad de los enlaces simbólicos en lugar de los datos. Esta función es totalmente compatible con los volúmenes de ONTAP FlexGroup y FlexVols. 
---
...
Además de la CLI de ONTAP, también puedes gestionar esta función mediante la API de REST de ONTAP. Consulta https://docs.netapp.com/us-en/ontap-automation/whats-new.html["Novedades de la API de REST DE ONTAP y la automatización"^] para obtener más información. La configuración que determina cómo procesa ONTAP los enlaces simbólicos de estilo Unix debe realizarse por separado para cada SVM.
