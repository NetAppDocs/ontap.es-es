...
summary: 'A partir de ONTAP 9.11.1 en plataformas de cabinas all-flash SAN (ASA), la función de conmutación por error de LIF iSCSI admite la migración automática y manual de LIF iSCSI en una conmutación al nodo de respaldo de un partner de SFO (cuando un LIF iSCSI pasa de su nodo/puerto principal a su nodo/puerto asociado de alta disponibilidad y de nuevo) y en una conmutación por error local (cuando un LIF iSCSI pasa de su puerto en mal estado a un puerto en buen estado de su nodo inicial actual y viceversa). Esta función proporciona una reanudación más rápida de las operaciones de I/o para las cargas de trabajo SAN que se ejecutan en ISCSI.' 
---
...
[role="lead"]
A partir de ONTAP 9.11.1 en plataformas de cabinas all-flash SAN (ASA), la función de conmutación por error de LIF iSCSI admite la migración automática y manual de LIF iSCSI en una conmutación al nodo de respaldo de un partner de SFO (cuando un LIF iSCSI pasa de su nodo/puerto principal a su nodo/puerto asociado de alta disponibilidad y de nuevo) y en una conmutación por error local (cuando un LIF iSCSI pasa de su puerto en mal estado a un puerto en buen estado de su nodo inicial actual y viceversa). Esta función proporciona una reanudación más rápida de las operaciones de I/o para las cargas de trabajo SAN que se ejecutan en ISCSI.
...
* La recuperación tras fallos y la migración de LIF iSCSI se producen dentro del dominio de retransmisión del puerto raíz de la LIF. Revise la configuración del dominio de retransmisión mediante el `network port broadcast-domain show` comando.
+

IMPORTANT: Asegúrese de que los dominios de retransmisión asociados a las LIF iSCSI contengan puertos de red que representen ambos nodos de un par de alta disponibilidad, de manera que las LIF iSCSI puedan migrarse y conmutar por fallo entre ambos nodos según sea necesario.
...
Consulte https://docs.netapp.com/us-en/ontap-cli-9141/index.html["Referencia de comandos de la ONTAP"^] para obtener una lista completa de `network interface` comandos.
