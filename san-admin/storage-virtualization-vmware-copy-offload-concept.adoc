...
Una LIF es una interfaz lógica que conecta la máquina virtual de almacenamiento (SVM) a un puerto físico. Aunque varias SVM pueden tener varias LIF en el mismo puerto, una LIF pertenece a una sola SVM. Puedes acceder a los LUN a través de las LIF de las SVM.
