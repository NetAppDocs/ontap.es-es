...
Un subsistema NVMe incluye una o varias controladoras, uno o varios espacios de nombres, uno o varios puertos de subsistema de memoria no volátil (NVM) (puertos de transporte FC-NVMe o RDMA), un medio de almacenamiento NVM y una interfaz entre las controladoras y el medio de almacenamiento NVM. Para la asignación y gestión de controladoras, un subsistema NVM se asigna a un Vserver en ONTAP.

Un subsistema NVMe over Fabric (NVMeoF) es un objeto kernel independiente que reside en el kernel FreeBSD. El subsistema NVMeoF interactúa con los siguientes componentes:

* Componentes SAN, como BCOMKA, FCT y VDOM
* WAFL
* Componentes RAS, como cm, ASUP y EMS


Todas las interfaces con subsistemas NVMeoF se adhieren a las definiciones y patrones actuales encontrados en ONTAP.
