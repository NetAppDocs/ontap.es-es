...
[role="lead"]
Cuando aprovisionas almacenamiento, habilitas tus hosts NVMe para leer y escribir datos en los sistemas de almacenamiento ONTAP. Para los sistemas que usan el protocolo NVMe, debes crear uno o más namespaces y subsistemas NVMe. Cada namespace se puede asignar a un subsistema NVMe para permitir el acceso a los datos desde tu host.

Si la máquina virtual de almacenamiento (SVM) que contiene el subsistema NVMe tiene interfaces de red NVMe/FC y NVMe/TCP, se puede acceder a los espacios de nombres asignados al subsistema NVMe mediante los protocolos de transporte NVMe/FC y NVMe/TCP. La compatibilidad de NVMe con FC está disponible a partir de ONTAP 9.4 y versiones posteriores. La compatibilidad de NVMe con TCP está disponible a partir de ONTAP 9.10.1 y versiones posteriores. Si necesitas acceso específico de protocolo a tus espacios de nombres, debes link:configure-svm-nvme-task.html["configurar una SVM"] para cada protocolo.
