...
[role="lead"]
Si no hay disponible un procedimiento para su host específico, puede usar estos pasos para aprovisionar almacenamiento para cualquier host SAN compatible.
...
. link:create-port-sets-binding-igroups-task.html["Cree un conjunto de puertos y enlace a un igroup"] (opcional).
