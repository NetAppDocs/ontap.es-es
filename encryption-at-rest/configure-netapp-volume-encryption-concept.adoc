...
+
** A partir de ONTAP 9.17.1, puede utilizar xref:manage-keys-barbican-task.html[Barbican KMS] para proteger las claves NVE solo para SVM de datos.
...
ONTAP
 a| 
Todas las implementaciones de ONTAP . La compatibilidad con Cloud Volumes ONTAP está disponible en ONTAP 9.5 y versiones posteriores.
...
Alcance de SVM
 a| 
MetroCluster es compatible a partir de ONTAP 9.8.

A partir de ONTAP 9.6, NVE admite el alcance SVM solo para la administración de claves externas, no para el administrador de claves integrado.
