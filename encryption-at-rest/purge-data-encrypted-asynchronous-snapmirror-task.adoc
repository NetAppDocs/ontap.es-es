...
summary: A partir de ONTAP 9.8, puede usar secure-purge para limpiar datos sin interrupciones en volúmenes habilitados para NVE con una relación asincrónica de SnapMirror . 
---
...
+
.. Crea una instantánea en el volumen de origen en la relación asíncrona de SnapMirror:
+
`volume snapshot create -snapshot snapshot_name -vserver _SVM_name_ -volume _volume_name_`

.. Actualiza la relación SnapMirror para mover la instantánea base hacia adelante en el destino:
+
