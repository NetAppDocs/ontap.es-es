...
El Onboard Key Manager almacena y gestiona las claves en una base de datos segura que es interna al clúster. Su alcance es el clúster. Un gestor de claves externo almacena y gestiona las claves fuera del clúster. Su alcance puede ser el clúster o la storage VM.

.Acerca de esta tarea
Debes usar la interfaz de línea de comandos (CLI) de ONTAP para configurar gestores de claves externos para configuraciones de MetroCluster. Conoce más sobre link:configure-external-key-management-concept.html["configurar la gestión de claves externas de ONTAP usando la línea de comandos"].

Se pueden utilizar uno o varios gestores de claves externos. Aplican las siguientes condiciones:
