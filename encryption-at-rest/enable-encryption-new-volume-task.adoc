...
* A partir de ONTAP 9.4, si se habilita `cc-mode` Cuando configura el anager de clave interna, los volúmenes que crea con el `volume create` el comando se cifra automáticamente, tanto si se especifica como si no `-encrypt true`.
...
|===
+
Para obtener una sintaxis completa del comando, consulte la página de referencia de comandos de LINK:https://docs.netapp.com/us-en/ontap-cli-9131/volume-create.html[`volume create`#].
