...
[role="lead"]
NetApp recomienda que uses System Manager para crear nuevos clústeres. System Manager ofrece un flujo de trabajo simple y fácil para la configuración de clústeres. Solo es necesario usar la interfaz de línea de comandos de ONTAP (CLI) si estás ejecutando ONTAP 9.7 o anterior en una configuración MetroCluster o si necesitas configurar un clúster solo IPv6 en sistemas compatibles.
...
La asignación de direcciones IPv6 a los LIF de gestión durante la configuración del clúster es compatible en los siguientes sistemas y versiones de ONTAP.

[cols="2"]
|===
| Versión de ONTAP | Sistemas compatibles 
...
Tu versión de BMC debe ser 19.3 o posterior para admitir la asignación de direcciones IPv6 durante la configuración del clúster.



| ONTAP 9.13.1 y versiones posteriores  a| 
* AFF A800
* FAS8700


|===
Debes realizar la configuración del clúster con la CLI de ONTAP para asignar direcciones IPv6 en los sistemas compatibles. Si la asignación de direcciones IPv6 durante la configuración del clúster no está soportada en tu sistema y versión de ONTAP, deberías usar System Manager para crear tu clúster usando direcciones IPv4 y luego link:convert-ipv4-to-ipv6-task.html["Convertir a IPv6"] direcciones después de completar la configuración del clúster.
...
.Antes de empezar
* Deberías haber instalado, cableado y encendido tu nuevo sistema de almacenamiento según las instrucciones de instalación y configuración para tu modelo de sistema.
+
