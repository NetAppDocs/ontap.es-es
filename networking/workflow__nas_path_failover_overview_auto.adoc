...
---
= Flujo de trabajo de conmutación por error de rutas NAS para ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: Una SAN LIF no migra (a menos que la muevas manualmente tras el fallo del enlace). En su lugar, la tecnología de multivía en el host desvía el tráfico a una LIF diferente. Para obtener más información, consulta link:../san-admin/index.html["Administración de SAN"].
