...
summary: Cumple con Federal Information Processing Standards 140-2 para todas las conexiones SSL. 
---
= Configura la seguridad de red de ONTAP usando FIPS
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
|===
Debes reconfigurar las cuentas de clave pública SSH existentes para usar un tipo de clave compatible antes de habilitar FIPS, o la autenticación del administrador fallará.
...
. A partir de ONTAP 9.9.1, no es necesario reiniciar. Si estás ejecutando ONTAP 9.8 o anterior, reinicia manualmente cada nodo del clúster de forma individual.
...
Puedes activar SSLv3 con el siguiente comando. Si estás ejecutando ONTAP 9.9.1 o posterior, no ves el mensaje de advertencia.
...
.Pasos
. Si estás ejecutando ONTAP 9.8 o anterior, reinicia manualmente cada nodo del clúster de forma individual.
...
* link:../networking/configure-tls-offload.html["Configura la descarga de hardware TLS"]
