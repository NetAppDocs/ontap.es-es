...
[role="lead"]
Una LIF (interfaz de red) es una dirección IP asociada a un puerto físico o lógico. Si hay un fallo de un componente, un LIF puede conmutar al respaldo o migrarse a un puerto físico diferente, lo que continúa comunicándose con la red.
...
. Compruebe que la LIF se ha creado mediante el `network interface show` comando.
...
====
.Más información
xref:modify_a_lif.html[Modificar una LIF]
xref:../nfs-rdma/configure-lifs-task.html[Configure LIF para NFS a través de RDMA]
