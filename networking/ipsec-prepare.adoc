...
summary: A partir de ONTAP 9.8, tienes la opción de usar seguridad IP para proteger tu tráfico de red. IPsec es una de varias opciones de cifrado de datos en movimiento o en vuelo disponibles con ONTAP. Deberías prepararte para configurar IPsec antes de usarlo en un entorno de producción. 
---
= Infórmate sobre la seguridad IP en la red ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Si usas ONTAP 9.16.1 o una versión posterior, tienes la opción de descargar ciertas operaciones que requieren mucho esfuerzo computacional, como el cifrado y las comprobaciones de integridad, a una tarjeta de interfaz de red (NIC) instalada en el nodo de almacenamiento. El impacto en el rendimiento del ancho de banda de red de las operaciones de cifrado IPsec descargadas a la tarjeta NIC es de aproximadamente 5 % o menos en comparación con las operaciones en texto claro sin cifrado IPsec habilitado. Esto puede mejorar significativamente el rendimiento y el ancho de banda del tráfico de red protegido por IPsec.
...
* X50131A (Módulo de E/S, 2PT, 100/200GbE)
* X50131B (Módulo de E/S, 2PT, 200/400GbE, B)
* X60132A (controlador Ethernet 4p, 10G/25G)


ONTAP 9.17.1 agrega soporte para las siguientes tarjetas Ethernet:

* X50135A (módulo de E/S, 2PT, 100GbE, Encrypt)
* X60135A (controlador Ethernet 2p, 40G/100G)


Las tarjetas X50131A, X50131B y X50135A son compatibles con las siguientes plataformas:
