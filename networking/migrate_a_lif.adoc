...
* Puede migrar LIF iSCSI en plataformas de cabina All SAN (ASA) que ejecuten ONTAP 9.11.1 o posterior.
+
La migración de LIF iSCSI se limita a los puertos del nodo principal o del compañero de alta disponibilidad.

+
También puede usar System Manager para migrar LIF iSCSI.link:../san-admin/asa-iscsi-lif-fo-task.html["Conmutación por error de LIF de iSCSI para plataformas ASA"]

* Si su plataforma no es una plataforma de cabina All SAN (ASA) con ONTAP versión 9.11.1 o posterior, no puede migrar LIF iSCSI de un nodo a otro.
+
