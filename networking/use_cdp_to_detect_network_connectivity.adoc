...
summary: En un centro de datos, puedes usar protocolos de descubrimiento de vecinos para ver la conectividad de red entre un par de sistemas físicos o virtuales y sus interfaces de red. ONTAP admite dos protocolos de descubrimiento de vecinos, Cisco Discovery Protocol y Link Layer Discovery Protocol. 
---
...
También debes habilitar CDP en todos los switches y routers Ethernet antes de que se pueda mostrar la información sobre los dispositivos vecinos.
...
NOTE: A veces, cuando se modifican los LIF en el nodo, la información del CDP no se actualiza en el dispositivo receptor (por ejemplo, un conmutador Ethernet). Si te encuentras con este problema, deberías configurar la interfaz de red del nodo primero en estado down y luego en estado up.
...
Varios conmutadores de proveedores no admiten CDP. Ver ellink:https://kb.netapp.com/onprem/ontap/da/NAS/ONTAP_device_discovery_shows_nodes_instead_of_the_switch["Base de conocimientos de NetApp : La detección de dispositivos ONTAP muestra los nodos en lugar del conmutador"^] Para más detalles.
