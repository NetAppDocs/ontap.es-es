...
* A partir de ONTAP 9.11.1, en las plataformas de cabinas All SAN (ASA), la conmutación por error de LIF iSCSI se habilita automáticamente en los LIF iSCSI recién creados en equipos virtuales de almacenamiento recién creados.
+
Además, puede habilitar manualmente la conmutación por error de LIF iSCSI en LIF iSCSI preexistentes, es decir, LIF que se crearon antes de la actualización a ONTAP 9.11.1 o posterior.link:../san-admin/asa-iscsi-lif-fo-task.html["Conmutación por error de LIF de iSCSI para plataformas ASA"]
