...
ONTAP S3 admite este encabezado adicional:
...
| PutBucketLifecycleConfiguration | ONTAP 9.13.1 * solo se admiten acciones de expiración 
...
A partir de ONTAP 9.19.1, ONTAP S3 admite la aplicación de escrituras condicionales para cargas de objetos y eliminaciones condicionales para eliminaciones de objetos. Las escrituras y eliminaciones condicionales proporcionan una forma de evitar sobrescrituras o eliminaciones involuntarias de objetos. Las operaciones condicionales solo funcionan si la solicitud se realiza mediante el método de firma AWS_V4 a través de una conexión TLS. Se admiten las siguientes cabeceras y operaciones de objetos condicionales:

|===
| Operación de objeto | Cabecera condicional 
...
ONTAP S3 admite este parámetro de consulta URI adicional:
`uploadId`
 a| 
ONTAP 9,8
...
ONTAP S3 admite estas cabeceras adicionales:
...
| DeleteObjects ONTAP S3 admite este encabezado adicional: * `x-amz-bypass-governance-retention` | ONTAP 9.11.1 
...
ONTAP S3 admite estos parámetros de consulta URI adicionales:
...
ONTAP S3 admite estos parámetros URI adicionales:
...
* ONTAP S3 añade compatibilidad con las solicitudes de firma de cargas por partes mediante `x-amz-content-sha256:
