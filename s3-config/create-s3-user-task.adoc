...
Para mantener el acceso a los recursos de S3 si la clave de acceso original caduca, a partir de ONTAP 9.19.1, puedes link:regenerate-access-keys.html#generate-a-second-s3-access-key["generar una segunda clave de acceso"] para cada usuario de S3.
...
.System Manager
--
. Selecciona *Storage > Storage VMs*. Selecciona la storage VM a la que necesitas añadir un usuario, selecciona *Settings* y luego selecciona image:icon_pencil.gif["Icono Editar"] en S3.
. Para añadir un usuario, selecciona *Users > Add*.
...
. Selecciona *Guardar*. Se crea el usuario y se generan una clave de acceso y una clave secreta para el usuario.
