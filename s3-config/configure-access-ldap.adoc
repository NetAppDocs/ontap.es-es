...
Si el grupo nasgroup especificado en la sentencia bucket-policy o los usuarios que forman parte del grupo nasgroup no tienen UID ni GID definidos, las consultas fallarán cuando no se encuentren estos atributos. Active Directory utiliza SID, no UID. Si las entradas de SID no pueden asignarse a UID, los datos necesarios deberán llevarse a ONTAP.

Para hacerlo, utilice link:../authentication/enable-ad-users-groups-access-cluster-svm-task.html["creación de directorio activo de vserver"] esta SVM para poder autenticarse con Active Directory y obtener la información de usuario y grupo necesaria.
...
NOTE: Dirija la API al LIF de gestión del clúster, no al LIF de datos de SVM.
