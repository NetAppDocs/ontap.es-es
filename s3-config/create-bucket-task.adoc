...
Para obtener más información sobre las definiciones de servicios de almacenamiento, consulte link:storage-service-definitions-reference.html["Definiciones de servicios de almacenamiento"]
...
Para obtener más información sobre la gestión del rendimiento, consulte link:../performance-admin/index.html["Gestión del rendimiento"]
...
.Ejemplo
En el siguiente ejemplo se crea un bloque para la máquina virtual de almacenamiento VS1 GB de tamaño 1TB y se especifica el agregado:
...
En aplicaciones cliente S3 (ya sea ONTAP S3 o una aplicación externa de 3^rd^), puede verificar el acceso al depósito recién creado introduciendo lo siguiente:
