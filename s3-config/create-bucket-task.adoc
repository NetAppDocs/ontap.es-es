...
*** Si no especificas los agregados y los componentes de un nuevo bucket, el nuevo bucket se añadirá a un FlexGroup ya existente, pero solo si la configuración `retention-mode` coincide con la configuración `retention-mode` de los buckets existentes y el tipo de FlexGroup (`compliance`, `non-snaplock`, `enterprise`). Consulta xref:../flexgroup/index.html[Gestión de volúmenes de FlexGroup] para más información.
+
...
Antes de ONTAP One, la licencia de SnapLock se incluía en el paquete de seguridad y cumplimiento de normativas. El paquete de seguridad y cumplimiento ya no se ofrece, pero sigue siendo válido. Aunque actualmente no es necesario, los clientes existentes pueden optar por link:../system-admin/download-nlf-task.html["Actualice a ONTAP One"]. , SnapLock link:../system-admin/install-license-task.html["instale"] antes de poder habilitar el bloqueo de objetos.
