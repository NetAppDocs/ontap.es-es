...
--
. Cree un grupo de S3:
`vserver object-store-server group create -vserver _svm_name_ -name _group_name_ -users _user_name\(s\)_ [-policies _policy_names_] [-comment _text_\]`La `-policies` la opción se puede omitir en configuraciones con un solo bloque en un almacén de objetos; el nombre del grupo se puede agregar a la política de bloque. La `-policies` la opción se puede añadir más adelante con la `vserver object-store-server group modify` comando después de crear las políticas de servidor de almacenamiento de objetos.
