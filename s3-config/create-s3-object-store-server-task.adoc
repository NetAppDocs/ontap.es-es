...
Al crear un servidor de almacén de objetos, se crea un usuario raíz con UID 0. No se genera ninguna clave de acceso ni clave secreta para este usuario raíz. El administrador de ONTAP debe ejecutar el comando  `object-store-server user regenerate-keys` para configurar la clave de acceso y la clave secreta de este usuario.
