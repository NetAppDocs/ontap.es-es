...
`vserver object-store-server user show`+ confirme que existe una clave de acceso para el usuario root. Si no lo hay, introduzca:
`vserver object-store-server user regenerate-keys -vserver svm_name -user _root_`+ no regenere la clave si ya existe.
