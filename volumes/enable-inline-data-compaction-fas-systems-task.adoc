...
summary: Habilita la compactación de datos inline en sistemas FAS con agregados Flash Pool o agregados HDD a nivel de volumen o agregado. 
---
...
[role="lead"]
Puedes activar la compactación de datos inline en sistemas FAS con agregados Flash Pool (híbridos) o agregados HDD a nivel de volumen usando el comando de shell de clúster  `volume efficiency`. La compactación de datos está desactivada por defecto para volúmenes creados en sistemas FAS. Conoce más sobre  `volume efficiency` en el link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=volume+efficiency["Referencia de comandos del ONTAP"^].
