...
* Convertir volúmenes FlexVol a volúmenes FlexGroup es una operación que no provoca interrupciones. La conversión normalmente tarda unos segundos, sin necesidad de desconectar el volumen ni volver a montar los recursos compartidos y las exportaciones, así que los clientes pueden seguir accediendo a los datos.
...
** En las versiones anteriores a ONTAP 9.10.1, el SVM donde se encuentra el volumen FlexVol que se va a convertir está usando actualmente SVM-DR. A partir de ONTAP 9.10.1, puedes link:convert-flexvol-svm-dr-relationship-task.html["convierte un volumen FlexVol en un volumen FlexGroup en una fuente SVM-DR"].
** NetApp FlexClone volúmenes están presentes, y el volumen FlexVol es el volumen primario. El volumen que se va a convertir no puede ser un volumen primario ni un clon.
...
** La protección autónoma contra el ransomware se ha desactivado en el volumen. No debes volver a activarla hasta que se haya completado la conversión.
...
[NOTE]
====
La conversión de un volumen FlexGroup muy grande da como resultado un volumen miembro constituyente FlexGroup muy lleno, lo que puede crear problemas de rendimiento. Para más información, consulta la sección llamada "Cuándo no crear un volumen FlexGroup" en la TR link:https://www.netapp.com/media/12385-tr4571.pdf["FlexGroup Volumes: Guía de prácticas recomendadas y de implementación"^].
